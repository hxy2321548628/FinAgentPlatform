...
步骤五留下的本地 `runs` 表样本如下，数值单位为 token：

| 样本 | cache | uncached | output | total |
|---|---:|---:|---:|---:|
| 对照组（不挂子智能体） | 7680 | 200 | 366 | 8246 |
| 实验组（挂子智能体） | 10368 | 3749 | 630 | 14747 |

实验组总 token 大于对照组，说明子图 token 已进入同一条 run 的本地计量。DeepSeek / Langfuse 后台账单在当前环境无法访问，**该项记为未验**；本地 `runs` 表不能冒充 DeepSeek 后台账单核对结果。
...
当前工作环境没有 Docker daemon、Postgres、Redis、broker、worker 组成的运行栈，因此 `P9①`、P9③、P9⑤、P9⑥以及依赖真实服务的历史判据不能在本次工作区验收中宣称通过；Playwright 已实际启动并走到登录页，但因后端未运行而记为未验。后续在完整 Compose 环境中运行 `bash deploy/test/verify.sh`，退出码为 0 才能写“47 条全过”；若仅有未验项，退出码必须是 2。
