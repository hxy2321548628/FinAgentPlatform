...
| 事实 | 出处 |
|---|---|
| **平台自身已跑通**（2026-08-10，P0–P4 五期验收全过）—— 但**接收能力一项都没实现**，见 §6 | [实施计划索引](../03plan/) |
| 「用户自定义提示词 / skill / MCP 接入」是**架构裁剪掉的本期范围** | [总体架构 §1.2、§7](./01architecture.md) |
| 该裁剪的重估触发条件是「**P0–P4 跑通，且**教师提出实际需求」—— 两个条件是**并且** | [总体架构 §7](./01architecture.md) |

**2026-08-10 更新：两个条件现在都满足了。** 后半条 2026-08-03 触发（有老师提出需求），前半条随 P4 验收通过而满足。

**但「可以接了」仍然不成立**，原因换了一个 —— 不再是「平台没跑通」，而是 §6 那五项接收能力一项都没做。**因此本文仍是规范而不是承诺。** 区别在于：此前它被一个未满足的前置条件挡着，现在挡着它的只剩排期本身，而排期已经到了该决定的时候（[风险登记 §10.3](./09risk-register.md) 的两笔债与它一起排）。
...
**这也正是[总体架构 §7](./01architecture.md) 早已认定的后续方向**，本文只是把它从「一个方向」细化成「一份规范」。
...
| 接入前**通读一遍代码** | 单人开发无第二人复核（[总体架构 §2.1](./01architecture.md)），这是唯一的审查环节 |
...
| 第三方工具的事件映射与前端渲染分支 | [运行时设计 §5.2](./05runtime-design.md) 事件契约 | 未实现（契约的兼容性规则已预留：前端须忽略未知 `type`） |
| skill / 提示词的存储与组内共享 | [数据设计 §6.2](./06data-design.md) 的 `agent_config` JSONB | 未实现。**「组内共享」还少一层**：`groups` / `user_groups` 于 P3 定案**不建**（没有共享资源就没有组的用武之地），要做组内共享得先把那两张表补回来 |
| 第三方服务的 compose 编排与资源限制 | [ADR-0001](./adr/0001-single-host-compose.md) | 未实现 |
| 工具级的配额计量 | [数据设计 §6.4](./06data-design.md) | 未实现 |

**排期建议**：接在 P4 之后作为 P5。**这个时点已于 2026-08-10 到达** —— P3 的配额计量与越权隔离（这条建议当初的依赖）已经就位，P4 也已收尾，因此「不建议早于 P3」这个限制不再起作用。**要不要做现在是一个纯粹的优先级决定**，与[风险登记 §10.3](./09risk-register.md) 的前端与效果评测两笔债放在一起排。

值得注意的是，架构里有两处**已经为此预留了位置**，说明这个方向早在设计时就被考虑过：

- `threads.agent_config` 用 JSONB 而非拆列，理由原文就写着「后续方向（自定义提示词、skill、MCP）会持续往里加字段」（[数据设计 §6.2](./06data-design.md)）
- 角色与课题组共享边界已定义，原文写明「是为了让表结构将来不必推倒重来」（[安全设计 §7.2.1](./07security-design.md)）
...
| 2 | **第三方工具的 token 消耗算谁的？** 算调用它的教师，还是算工具作者 | 不再阻塞：[数据设计 §6.4](./06data-design.md) 的配额口径与数值已于 P3 落地（数值仍是待校准的初值，见[风险登记 §10.3](./09risk-register.md)）。这一问现在纯粹是**政策选择**，可以直接答 |
| 3 | **组内共享还是全院共享？** 一个老师的 RAG 工具默认对谁可见 | **前提变了**：[数据设计 §6.2](./06data-design.md) 已于 P3 定案**不建** `groups` / `user_groups`（理由正是「没有共享资源就没有组的用武之地」）。要选「组内共享」就得先把那两张表补回来 —— 这一问因此从「选哪个」变成了「值不值得为它建两张表」 |
| 4 | **凭据注入的具体形式** —— §3.6 提到平台注入 LLM 凭据，机制未定 | 依赖 §6 的实现 |
| 5 | **长耗时工具的进度反馈** —— 超过 30 秒的调用如何推事件 | 不再阻塞：平台自身的 `sandbox.queued` 机制已于 P1 落地（`app/event/model.py`、`app/sandbox/pool.py`），第三方工具照它同一套做即可 |
...
| §3.6 隔离与配额 | [数据设计 §6.3 §6.4](./06data-design.md) |
| §3.7 资源账 | [运维设计：部署拓扑与容量输入](./08operation-design.md) 容量推算 |
| §4 安全边界 | [安全设计 §7.1](./07security-design.md) 威胁模型、[ADR-0004](./adr/0004-sandbox-broker-docker-sock.md) |
| §6 平台侧能力 | [总体架构 §7](./01architecture.md) 的重估触发条件 |
