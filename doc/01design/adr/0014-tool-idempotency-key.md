...
| 决策人 | hxy |
| 关联设计 | [总体架构 §2.3](../01architecture.md)、[运行时设计 §5.6](../05runtime-design.md) |
...
**去重放在 broker 而不是 worker**：broker 是沙箱的唯一入口（[ADR-0004](./0004-sandbox-broker-docker-sock.md)），而 worker 会崩溃重启 —— 把去重状态放在 worker 侧，它正好在最需要它的那一刻（崩溃恢复）跟着进程一起没了。
...
| **worker 侧去重** | worker 会崩溃重启，去重状态放在它那边，正好在最需要它的那一刻（崩溃恢复）跟着进程一起没了 |
...
## 落地补记（2026-08-08，P3 步骤六）

已实现，落点与上文的定案一致：worker 侧的 backend 在四个写操作上从 `get_config()["configurable"]` 读出 `checkpoint_ns` 随请求发出（[`sandbox/remote.py`](../../../app/sandbox/remote.py)），broker 侧按 `(thread_id, checkpoint_ns)` 去重（[`broker/cache.py`](../../../app/broker/cache.py)）。

三处实现时才定下来的细节：

- **错误结果也缓存。** 只缓存成功等于没解决问题 —— 重放时 `old_string` 已经不在、文件已经没了，那个错误正是首次执行没有的。
- **拿不到键时去重关掉而不是报错。** 工具也会在图之外被调用（测试、将来的管理动作），那时没有 LangGraph 的上下文。少一层去重只是回到没有它的从前。
- **去重表连不上时降级并告警。** 它是崩溃路径上的一道保险，不是执行的前提；为了保险失灵就让每一次写操作都 500，代价大得多。broker 启动时另有一次体检，配错了在部署那一刻就暴露。

上文「键改成了框架内部概念」那条残留风险，**已做成可重跑的用例**：`test/broker/cache_test.py` 里那条并行场景断言同一轮的两个调用拿到不同的 ns。若 LangGraph 改变扇出粒度，它会红 —— 而没有它，第二次调用会静默地拿到第一次的结果。**升级 langgraph 时必须重跑。**
