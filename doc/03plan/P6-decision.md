...
| 作者 | hxy |
| 日期 | 2026-08-13 |
| 下游文档 | P6-plan.md ~ P10-plan.md（按 §9 的 L1 分期，尚未撰写） |
...
### J2 SSE 客户端 —— **定案：`EventSource` + `Last-Event-ID`，封成 hook**

后端 `GET /api/runs/{id}/events` 是整个平台的核心资产（断线重放、已终态 run 先补历史再自然收尾），**前端至今一行都没用过**。
...
>
> **上面那条 ⚠️ 只兑现了一半**：免费的 14 条已在真机上全过（2026-08-14，XFS 正常挂载下）；
> **要花钱的 7 条被 `HTTP 402 Insufficient Balance` 挡住**（DeepSeek 账户余额不足，外部原因，
> 记「未验」而不改判绿），**要 root 的 1 条未跑**。`p5.sh` 那四条也仍未补 —— 补的形式变成
> 「在这个文件里加一个 P5 段」，不再是新写一个脚本。
>
