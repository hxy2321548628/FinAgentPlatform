...
| 项 | 值 |
|---|---|
| 文档状态 | **开发完成，两条主判据都成立**（2026-08-16）。实施记录见 [§8](#8-实施记录) |
| 当前版本 | v0.2 |
| 作者 | hxy |
| 日期 | 2026-08-15 |
| 一句话验收 | **外网工具真被调用** |
...
| 观察项 | 计划里的预期 | 实测 |
|---|---|---|
| `isError` 出现几次（§1.4 第 5 条） | 长期为 0 则熔断口径可以收紧 | **0 次**（第一个样本） |
| 装配多花多少（§1.4 第 7 条） | 本地夹具 39 ms、真外网 3.3 秒 | 本地夹具约 40 ms，**但曾经是两次**，见 §8.3 |
| `ANALYSIS_TOKEN` 第 19、20 个样本（§7 第 4 条） | 「MCP 不放大 token，反而可能减少」 | **方向相反，而且两次都是**：步骤六手工那次不挂 `7424｜111｜223` / 挂上 `11648｜411｜304`，收尾回归那次不挂 `11264｜237｜299` / 挂上 `11648｜399｜295` —— 工具 schema 进了每一轮 prompt，挂 MCP 反而多烧。§7 第 4 条那条待决事项已按这个结果重写 |

### 8.6 收尾的那一轮全量回归

2026-08-16，`SKIP_HOSTILE=1 bash deploy/test/verify.sh`（六个服务起着、真 DeepSeek、夹具由脚本
自己起停）：**54 条里 53 条通过，1 条未验**。
...
- **唯一未验的是 `P1①`**（四条破坏性测试要交互式 sudo，跑不进无 tty 的会话）——
  **记「未验」而不是「通过」**，与前几期同一条规矩。退出码因此是 `2`。

`P10①` 这一轮的 token 样本与手工那次同向（不挂 `11264｜237｜299`，挂上 `11648｜399｜295`），
两次都是**挂 MCP 更贵**。
