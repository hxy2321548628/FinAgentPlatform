...
> **`reasoning` 是独立事件，必须与 `token` 分开渲染**（2026-08-02，依架构文档 §5.2 的 P0 实测回填）。主模型 `deepseek-v4-pro` 的思考过程与正式答复是两个字段、交替流出。若合并渲染，教师看到的会是思考与结论混在一起的一段文字。
>
> 这一条又反过来印证了本节的选择：成品 runtime 协议未必有「思考过程」这个通道，自研渲染层可以直接给它一个折叠区。
