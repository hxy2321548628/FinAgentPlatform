...
| 项 | 值 |
|---|---|
| 文档状态 | **开发完成**（2026-08-15），实施记录见 §8 |
| 当前版本 | v1.0 |
| 作者 | hxy |
| 日期 | 2026-08-15 |
| 一句话验收 | **挂与不挂同一个 skill，产物里看得出差别** |
| 第二条主判据 | **八条校验是闸门不是提示**：八个恶意包逐个被拒，且磁盘上不留一个字节（见 §1.1） |
...
| v1.0 | 2026-08-15 | hxy | 开发完成，补 §8 实施记录。九个步骤逐步提交；八条校验、动态重载、agent 自带 Skill、四页前端与两条浏览器链路全部落地。收尾按实测把 `.html` 纳入附件白名单，并补上计划已定案的平台 Skill 模板约束 |
...
八条校验因此不是提示而是闸门。前端早就把话说出去了（「解压后不超过 5 MB」「校验路径穿越、符号链接、文件数量、压缩比和文件类型」），[D3](./P6-decision.md) 也把阈值定完了，而**这些现在一条都没实现**。一条只在界面上写着的校验，与没有校验的区别只是心里踏实。
...
|---|---|
| 状态 | **开发完成，两条主判据都成立** |
| 双端门禁 | `make all` 全绿：后端 **1122** 条用例、前端 **123** 条 |
| 判据总数 | 35 → **41**，免费 25 → **29**，LLM/root 10 → **12** |
| 第一条主判据 | ✅ `P8①` 双向实测通过：不挂 Skill 的图为 `outputs/volatility_bar.png`，挂上后为 `outputs/ANNUALIZED-252-returns-volatility.png`；两个会话的 `skill/` 文件与 run 快照也分别符合预期 |
| 第二条主判据 | ✅ `P8②` 八个恶意真包逐个被拒，理由可区分，磁盘目录数不变；500 MB 解压量的压缩比包在解压前被拒 |
| 回归验收 | 免费轮 29 条全过；LLM/root 轮中 P0–P7 已验项及 `P8②`–`P8⑥` 通过，`P8①` 修正后另行双向复跑通过；`P1①` 因需要交互式 `sudo` 本轮未验，**不记作 41 条全过** |

### 8.1 九个步骤的落点与提交

| 步骤 | 落点 | 提交 |
|---|---|---|
| 一 数据层与迁移 | `app/preset/skill.py`、`app/preset/model.py`、迁移 `0012_skill_catalog` 与约束测试 | `0424ed2` |
| 二 上传包安全校验 | `app/preset/skill_package.py`，以真 ZIP 覆盖八条拒绝规则 | `471352b` |
| 三 Broker 存储与对齐 | Skill 版本落盘、按内容 hash 全量覆盖、保留目录防护 | `2dd0e59` |
| 四 目录管理与审核 | Skill CRUD、版本发布、可见性、共享、提审与 reviewer 审核 | `c4738ad` |
| 五 运行配置与动态重载 | 配置快照、提交侧引用解析、每次 run 前对齐、`ReloadingSkillsMiddleware` | `15e9070` |
| 六 智能体自带能力 | agent 版本冻结 Skill 引用，合并时处理撞名与 10 个上限 | `9be6e18` |
| 七 前端能力管理 | `MySkills`、能力广场、管理员审核、创建智能体与聊天配置接真 API | `9289c08` |
| 八 浏览器链路 | `web/e2e/skill-upload.spec.ts` 覆盖越界拒绝、合法上传、发布、提审与审核 | `d6f6c9f` |
| 九 验收与文档 | `verify.sh` 加 `P8①`–`P8⑥`，校正文档漂移并记录实测证据 | 本次收尾提交 |

### 8.2 与计划不同的四处

1. **ZIP 附件白名单增加 `.html`。** 用户用真实 `skill-creator.zip` 上传时，包内的
   `assets/eval_review.html` 与 `eval-viewer/viewer.html` 被拒。Skill 经常需要随包携带本地静态说明页，
   因此 `.html` 与 Markdown、文本、Python、结构化数据一样允许作为 ZIP 内附件；上传入口仍只有 ZIP
   与单 Markdown，不接受单独上传 HTML。
2. **收尾审计补上平台自定义 Skill 提示词模板。** 原装配虽然用了
   `ReloadingSkillsMiddleware`，却沿用了 DeepAgents 默认模板，遗漏了 §2 第 4 条要求的优先级约束。
   现在模板末尾明确写明：Skill 说明不得改变平台工作方式，冲突时以平台约束为准，并由工厂单测盯住。
3. **`P8①` 的提示词与等待窗口收紧。** 原提示词同时要求分析、绘图和多项解释，无 Skill 一侧在
   600 秒跑满后被取消。判据改成只要求完成一个波动率柱状图任务，并且只将 `P8①` 的窗口提高到
   900 秒；其余真实分析仍保留 600 秒，避免用统一放宽掩盖回归。
4. **浏览器合法包带一份真实 HTML 附件。** `P8⑥` 不再只证明 Markdown 能上传，而是把
   `assets/eval_review.html` 放进合法 ZIP，直接覆盖本次用户实测暴露的场景。

### 8.3 `P8⑥` 的红绿证据

这条链路不是只在最终状态跑绿：

1. 开工时把越界包“不新增”的断言故意写成 `toHaveCount(1)`，实际收到 0 并跑红；恢复成
   `toHaveCount(0)` 后通过，证明断言确实执行到了列表结果。
2. 曾临时破坏路径穿越校验，浏览器链路随即在“显示具体拒绝理由”处变红；恢复校验后重新通过。
3. 将 `.html` 加入白名单后，合法 ZIP 增加 `assets/eval_review.html`，再次运行完整链路：越界包被拒且
   不新增，带 HTML 的合法包成功创建、发布、提审并由 reviewer 审核通过，Playwright **1 passed**。

### 8.4 `P8①` 双向实测与 token 样本

修正提示词后使用两个独立会话复跑：

```text
无 Skill：
thread = 6d483461cda5453bac078d2201aa877a
run    = 0bd38824-dd92-4239-a534-eda6bcb44da4
产物   = outputs/volatility_bar.png
ANNUALIZED-252- 前缀数 = 0

挂 Skill：
thread = 867cdb4fb5464bbf96e6b616cc5e03a8
run    = 285bda45-f421-426d-99f5-79f1a5dda4b7
产物   = outputs/ANNUALIZED-252-returns-volatility.png
ANNUALIZED-252- 前缀数 = 1
```

双向断言成立：未挂的一侧没有前缀且 workspace 没有 `skill/`；挂上的一侧有
`skill/annualized-naming/SKILL.md`，run 快照含 `skill_id`、`version`、`name`，产物文件名带约定前缀。

这两条真实分析是 `ANALYSIS_TOKEN` 的第 15、16 个样本：

| 样本 | run | cache read | 未命中 | output |
|---|---|---:|---:|---:|
| 15（无 Skill） | `0bd38824-dd92-4239-a534-eda6bcb44da4` | 14,464 | 514 | 2,045 |
| 16（挂 Skill） | `285bda45-f421-426d-99f5-79f1a5dda4b7` | 13,184 | 3,678 | 983 |

### 8.5 验收汇总

`verify.sh` 现有 **41 条**判据：29 条免费、12 条需要 LLM 或 root。

- `SKIP_LLM=1 SKIP_HOSTILE=1 bash deploy/test/verify.sh`：29 条免费判据全部通过，12 条明确记为
  “未验”，退出码 2；`P8②`、`P8④`、`P8⑤`、`P8⑥` 均在其中通过。
- `SKIP_HOSTILE=1 bash deploy/test/verify.sh`：P0–P7 的已验项通过，`P8②`–`P8⑥` 通过；旧版
  `P8①` 在 600 秒窗口内未完成，随后按 §8.2 第 3 条修正并用 §8.4 的两个会话单独复跑通过。
- `P1①` 的四个破坏性场景需要交互式 `sudo`，本轮显式跳过，因此当前证据是“除 `P1①` 外均有
  通过记录”，而不是一次性 41 条全过。
...
```bash
UV_CACHE_DIR=/tmp/uv-cache make all
```

结果：后端 **1122 passed**，前端 **123 passed**，双端门禁通过。
