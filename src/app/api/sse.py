...
`id:` 那一行是断线重连的全部依据 —— 客户端在同页内把最后成功处理的 id
保存为内存游标，重连时主动放进 `Last-Event-ID` 请求头，服务端据此补齐
中间的事件。整页刷新不复用该游标，从头重放。少写这一行，同页重连就只能
从头开始或者干脆漏事件。
"""
...
# 心跳走 SSE 注释行（冒号开头）。**不能给它 id** —— 否则客户端会把心跳
# 当成可续读位置，重连时就从心跳之后开始读，中间的真事件全被跳过。
