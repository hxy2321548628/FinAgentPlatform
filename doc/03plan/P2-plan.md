...
### 7.1 幂等键：实测结果与据此的决定（2026-08-07 定案，2026-08-14 补两轮并更正读法）

验收①跑了四轮崩溃恢复，脚本打印的观察项分别是：
...
> 前两轮**什么都没量到**，样本数实际是 **2 而不是 4**。
>
> 后两轮的 `run.started` 都是 2 条，是这项观察真正的两个样本。
...
- **只有两次真的砍到，且崩溃点都是随机的。** `kill -9` 挑在 run 进入 `running` 之后约 20 秒落下，砍到的那两刀都落在了两次工具之间。砍进 `execute` 执行途中的那一刀会是什么样，四轮都没有覆盖到。
