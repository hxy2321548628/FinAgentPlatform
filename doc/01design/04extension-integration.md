...
- **执行层**（沙箱容器）：**不可信区**，可出网但无 root、rootfs 只读，故障只影响单个会话
...
| **爬虫** | B / C / D 都行 | P12 开放沙箱出网后，沙箱里跑爬虫是可行的 —— 但要自己扛超时与重试，见 §3.3 |
...
**前提 —— 纯 Python**：沙箱内非 root 且 rootfs 只读，`apt-get` 一定失败。库若依赖系统级组件（编译器、系统 so），只能预装进镜像，不能让 agent 现装。

联网**不再是**形式 B 的禁区（P12 起沙箱可出网），但联网取数的库仍要自己处理超时与重试 —— 沙箱里一次执行有 wall-clock 上限，卡住就是整轮失败。

**两条落地路径**：

| 路径 | 适用 | 代价 |
|---|---|---|
| 发布到 pypi（或镜像源可达处） | 更新频繁、只有部分课题组用 | agent 需要 `pip install`，占 5GB workspace 配额 |
...
| 项 | 内容 |
|---|---|
| **要求** | 两层都能出网（沙箱自 P12 起）。选层看的是**故障影响面与执行时长**，不再是能否联网 |
| **为什么** | 沙箱里一次执行有 wall-clock 上限、无 root、且失败只影响单个会话；应用层没有执行时长约束，但一处卡死影响全平台 |
| **怎么选** | 取数快、只服务当前分析 → 沙箱内（形式 B）。耗时长、要缓存复用、或要连内部服务 → 应用层（形式 C / D） |

**「爬虫」类需求的注意点**（P12 之前这条路是死的，现在通了，但坑换了一个）：

> 沙箱里能跑爬虫，但**单次执行超时是硬上限**。抓几百页的活儿不要指望在一次 `execute` 里跑完 —— 要么切小分批跑，要么放应用层。
...
| # | 交什么 | 说明 |
|---|---|---|
| 5 | **接口描述**：MCP tool schema | 工具描述文本是给 LLM 读的，写清楚「什么时候该用它」比写清楚参数更重要。**平台会在装配时把实际拿到的工具名与这份清单比对，不一致记一条 WARNING**（[F6](../03plan/P6-decision.md)）—— 你改了工具名要来更新它。**不一致不会让整个 server 被跳过**（2026-08-16 由 P10 落地时改的：拦下来会让一次正常的上游升级打断所有人的分析），但**与平台内置文件工具重名的那个工具会被当场剔除** —— 实测外部工具会静默顶掉内置的，那是安全问题不是整洁性问题 |
| 6 | **耗时声明**：单次调用的典型耗时与最坏耗时 | 平台按 **30 秒硬超时**执行（[F5](../03plan/P6-decision.md)），超过的调用一律算失败。**这道超时是平台自己包上去的**，不是传给 MCP 客户端的那个参数 —— 后者管不到 SSE 的读等待，实测一台「接受连接但不回应」的服务能挂住 300 秒（[P10 §2.2](../03plan/P10-plan.md)）。超时返回的是一条错误结果给模型，不会掀掉整次分析 |
...
> **熔断的口径（2026-08-16 由 P10 定案）**：**只数传输层失败与超时** —— 连不上、连上了不回应。
> 你的工具自己返回错误（`isError`）**不计数**，因为模型传错参数在一次分析里连着发生 5 次很常见，
> 那样会把一个健康的服务停掉。**代价是**：一个「HTTP 200 但永远返回错误」的服务不会被自动停用，
> 它会一直挂在目录里。连续 5 次传输层失败即自动停用，此后平台**一次都不会再连它**，恢复要管理员
> 手动点 —— 修好之后请联系管理员。
