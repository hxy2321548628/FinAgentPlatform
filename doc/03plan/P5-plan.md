...
**补法（P6 开工前）**：`p5.sh` 至少覆盖四条 —— 注册+入组+审批走通、工作目录五个端点走通、会话增删改查与历史走通、删会话之后目录真的没了；并按惯例转调 `p4.sh`。
...
**产物与工作目录是两条路，不是重复。** 产物的身份长在 `artifacts` 表上、字节在对象存储里，因此下载走 nginx 直发；工作目录说的是「此刻长什么样」，字节只在宿主机的 workspace 里，只能经 api 一段段转出去。同一张图两条路都取得到，那是它确实既是文件也是产物。
