...
> | | 给谁 | 状态 |
> |---|---|---|
> | **运维告警**（服务挂了、错误率飙升） | 管理员 | **已接飞书并验收过**（P4 §8.8、`deploy/grafana/provisioning/alerting/contact-point.yml`），**本期不动** |
...
**告警复用 P4 已接的飞书运维通道** —— 那条路已经验收过（P4 §8.8），不必新建。注意这是**运维告警**，与 C6 定的「用户侧不接飞书」不冲突。
