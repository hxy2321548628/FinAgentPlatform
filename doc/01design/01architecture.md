...
| v0.8 | 2026-07-31 | hxy | 新增 §7.3.5 磁盘与 tmpfs 配额（`/workspace` 5GB + `/tmp` 512MB），关闭磁盘配额缺口；§7 全章完成 |
...
| 前提 | 取值 | 影响 |
|---|---|---|
| **规模** | 一个学院的教师 + 其课题组研究生，合计约一两百人（§7.2.2） | 单机部署即可，不上 K8s / 队列分片（[ADR-0001](./adr/0001-single-host-compose.md)） |
...
**关于 `student` 角色的范围（2026-07-31 确认）：仅限课题组内的研究生，不面向全院学生开放。**

按每位教师带数名研究生估算，全院用户总数约一两百人 —— 与原先「几十到几百名教师」处于同一量级。因此 §7.2.2 引入学生角色**不改变任何架构结论**：§3.2「不必分布式」的权衡、§8.1 的 20 个沙箱上限、[ADR-0001](./adr/0001-single-host-compose.md) 的单机结论均维持不变。

> **这是一条需要守住的边界。** 若将来放开到全院学生（可能上千人），用户基数增加一个数量级，上述三处**都必须重新评估**，且 LLM token 成本会由学生侧主导（§6.4）。已登记入 §10.2 风险与[附录 B](#附录-b已评估但本期不采用)。
...
**这三条一起解除了架构中最大的一个不确定性** —— §4.3 的公有云模型选型与 [ADR-0009](./adr/0009-default-model-selection.md) 成立，不需要再为「转内网私有化部署」准备退路（该退路作为对冲仍保留在 ADR 中，但不再是预期路径）。

由此可以删掉的设计（原本因合规未定而被迫留白，见 §7.4）：

- 发往 LLM 的数据**不需要**脱敏
- MinIO / Postgres **不需要**静态加密
- §6.5 的数据保留期由运维成本决定，不再由合规期限决定

技术通路是另一件独立的事，也已确认：内网可访问 `api.deepseek.com`（§10.1）。**合规准许与网络可达两者都成立**，公有云模型方案再无前置条件。
...
> **已决（2026-07-31）：本期不设量化 SLA。** 依据 §1.1 —— 本期目标是端到端跑通，学院侧未提出可用率、延迟或 RPO/RTO 的预期，凭空拍数只会得到一个没人对其负责的数字。
>
> 上表的**定性排序仍然有效，且是全文的决策依据**，缺的只是数字。
>
> 这一决定有一个具体后果：§8.2 中「Postgres 单点是否需要主从」**无法再用指标论证**，只能按成本与故障代价判断。见 §8.2。
>
> 回填时机：平台真实投用后用实际观测值反向定义。
...
**TLS 方案（2026-07-31 确认）：走 HTTP，不启用 TLS。**

用内网 IP 直连意味着没有域名、签不了受信证书，剩下的两个选择中自签证书会让浏览器每次弹红色警告（除非把根证书推给全院），教师体验代价明显；而内网部署、用户可信、无合规要求（§2.4）使明文传输的风险可接受。SSE 本身不要求 secure context，功能上无影响。

**这一决定对 §7.2.3 认证方案有一个直接后果**：Session Cookie 无法设置 `Secure` 标志，凭据在内网链路上是明文的，同网段抓包即可窃取会话。在「内网 + 实名教师 + 无敏感数据」的威胁模型（§7.1）下接受这一风险 —— 但它是一条**真实存在的风险，不是零成本**，记入 §10.2。

> **重新评估的触发条件**：需要用到依赖 secure context 的浏览器 API（剪贴板、通知、Service Worker 等）；或平台开放到内网之外；或开始承载敏感数据。届时应优先申请域名 + 校内 CA 证书，而不是退回自签。
...
**管理员不例外。** §7.2.2 规定管理员不可查看他人的会话内容与上传数据，因此管理员身份**不应该**在数据访问层被实现成「绕过过滤」的旁路。这条边界最容易在后期为了「方便排查问题」而被悄悄破坏，一旦破坏就很难再收回来。

### 6.4 配额

LLM 调用是真实成本。至少需要 per-user 的 **token 日配额** + **并发 run 数上限**，否则单个用户就能占满整个 worker 池和沙箱池。

**必须按角色分级**（§7.2.2 确认后新增）。`teacher` 与 `student` 在权限上完全相同，**配额是二者唯一的实质差别**，也是控制成本的唯一手段 —— 学生人数通常远多于教师，若配额相同，成本结构会由学生侧主导。
...
> §7.2.2 引入 `student` 角色后这一判断**基本不变** —— 学生同样是校内实名用户。但需注意学生的**主动探测意愿**通常高于教师（好奇心驱动的沙箱逃逸尝试），这提高了 §11 中 P1 沙箱加固的必要性，不宜再往后拖。

### 7.2 认证与授权

#### 7.2.1 账号体系

**决策（2026-07-31）：自建账号体系，不对接学校统一身份认证。**

代价是要自己管密码、找回与账号生命周期；收益是不依赖信息化部门的对接排期，也不受其接口可用性约束。在本期「先跑通」的定位（§1.1）下这个交换划算。
...
| **教师 `teacher`** | 仅自己的 | 所属组**可读可写**（可发布 skill、提示词） | — |
| **学生 `student`** | 仅自己的 | 所属组**可读可写**，与教师同权 | — |

**管理员不能查看他人的会话内容与上传数据。** 这条边界要落在数据访问层，理由见 §6.3。

**课题组（group）** 是共享单元，**一个用户可同时属于多个组**（教师与学生皆是）。因此：

- §6.2 需要 `user_groups` 关联表，而非 `users.group_id` 单外键
- 共享资源的可见性判定**走并集** —— 用户能看到其所有所属组的共享资源之和
- 不引入「组内角色」（组长 / 成员）。读写权限由**全局 role** 决定，不按组区分。这是刻意的简化：组内角色会让权限判定从「查 role」变成「查 role × 查该组内身份」，复杂度翻倍，而当前没有需要它的场景

组内共享的**仅限配置类资源** —— 成员上传的 skill、配置好的智能体提示词。**不共享**会话、上传的数据文件、产物。

> 组内共享的 skill、用户自定义系统提示词、MCP 接入**本期均不实现**（§1.2）。本节先把角色模型和共享边界定下来，是为了让 §6.2 的表结构将来不必推倒重来。

**关于共享写权限（2026-07-31 确认）**：学生与教师对组内共享资源同权，均可读可写。

即 `teacher` 与 `student` 在本平台的**权限完全相同**，区别只在配额档位（§6.4）与统计口径。这是有意为之 —— 课题组的语义是协作而非管理层级，让学生也能贡献 skill 与提示词符合实际使用方式。

代价是没有质量把关，组内可能积累低质量配置。应对方式是记录 `created_by`（§6.2）使来源可追溯，而不是加审核流程 —— 几人到十几人的课题组内，社交约束比技术约束有效。

#### 7.2.3 认证方式

**决策（2026-07-31）：HttpOnly Cookie + 服务端 Session（存 Redis）。不用 OAuth2，不用 JWT。**

| 项 | 取值 |
|---|---|
| 凭据载体 | HttpOnly + SameSite=Lax Cookie |
| Session 存储 | Redis（已是必需组件，§6.1） |
| 有效期 | 7 天滑动过期 |
| 多端登录 | 允许并存（教师在办公室和家里都会用） |

**为什么不是 OAuth2**：OAuth2 解决的是「**第三方应用**代表用户访问资源」的授权委托问题。这里没有第三方 —— 账号自建（§7.2.1），前端和后端是同一个系统。硬套 OAuth2 等于自己同时扮演 authorization server、resource server、client 三个角色，为一个不存在的问题引入三套概念。

> 一个容易混淆的点：FastAPI 的 `OAuth2PasswordBearer` 只是借用了 OAuth2 password grant 的表单格式，用它并不等于实现了 OAuth2。若需求本来就是「登录拿个 token」，那要的从来不是 OAuth2。

**为什么是 Session 而不是 JWT**：

- **可主动失效** —— 管理员禁用账号、调整配额、强制下线都能立即生效。JWT 做不到，得再补 refresh token + 黑名单，复杂度反而更高。这是决定性理由
- JWT 的核心收益是无状态、便于水平扩展，这一条在「单机、几百用户」（§2.2）下价值为零
- Redis 已在架构里，Session 零新增依赖
- HttpOnly + SameSite 使凭据不暴露给 JS，天然规避 XSS 窃取

**两个实现上必须注意的点**：

1. **SSE 的凭据携带** —— 前端已决定用 `@microsoft/fetch-event-source`（为了 `Last-Event-ID`，见 §5.2），它默认**不带**凭据，须显式配置 `credentials: 'include'`，否则 SSE 请求会 401。
2. **Cookie 无 `Secure` 标志** —— §4.4 决定走 HTTP，因此凭据在内网链路上是明文的。已按 §7.1 威胁模型接受，记入 §10.2。
...
这两条堵的是 §7.1 威胁表里「写满磁盘」那一格，是加固清单里最后一个缺口。

**`/workspace` 配额：5GB / thread（2026-07-31 确认）**

`--read-only` 保护的只是 rootfs。`/workspace` 是可写 bind mount（§5.5），LLM 生成的代码可以往里写满宿主机磁盘。

用 **XFS project quota** 实现：

```bash
# 前提：承载 /data/sandbox 的文件系统以 XFS + prjquota 挂载
xfs_quota -x -c "project -s -p /data/sandbox/{thread_id} {projid}" /data
xfs_quota -x -c "limit -p bhard=5g {projid}" /data
```

`projid` 由 sandbox-broker 维护 `thread_id → 数字 id` 的映射（`sandboxes` 表加一列即可）。

被放弃的两个备选：

| 方案 | 放弃理由 |
|---|---|
| **Docker `--storage-opt size=`** | **它限制的是容器可写层，不是 bind mount** —— 对我们的 `/workspace` 完全无效。这个误解很常见 |
| **每 thread 一个 LVM 卷 / loop 设备** | 创建慢、数量受限、需要特权操作。per-thread 动态创建的场景下不合适 |

**`/tmp` 必须限容 —— 这一条比磁盘配额更急。**

`--read-only` 意味着 `/tmp` 只能挂 tmpfs，而 **tmpfs 吃的是宿主机内存，不是磁盘**。不限容的话，一句 `dd if=/dev/zero of=/tmp/x` 就能把宿主机内存写满，触发 OOM killer —— 而 §4.4 特意留 4 个沙箱余量防的正是 OOM killer 误杀 Postgres。**不限 tmpfs 等于那份余量白留了。**

故限 `size=512m`，且这 512MB 应计入 §8.1 的单沙箱内存预算（2GB 上限之内，不是额外的）。

**一个容量上的提醒：配额不等于总量有界。**

按 §5.5，容器销毁后 **workspace 留在卷里**，因此磁盘占用不是「20 个活跃沙箱 × 5GB = 100GB」，而是「历史 thread 总数 × 最多 5GB」。几百用户各开几十个会话，worst case 是 TB 级。

§4.4 的「磁盘充裕」**不解除这个问题** —— 单纯扩容只是把撞墙时间往后推。真正的解法是 §6.5 的保留策略：workspace 归档到 MinIO 后删除本地副本，下次需要时再拉回。已在 §6.5 登记。

> **实现提醒**：`pip install` 装的包也落在 workspace 里（rootfs 只读，装不进 site-packages），而 pandas / numpy / scipy / matplotlib / statsmodels 这一套就要 1–2GB。建议**把常用科学计算栈预装进沙箱镜像**，让 `pip install` 只用于额外的包 —— 否则 5GB 里有小一半被基础包吃掉，且每个 thread 都要重装一遍。
...
> 注意这一条**没有**被合规结论解除 —— 它的驱动因素不是合规，而是 §7.2.2 中管理员权限较大，需要可追溯。
...
| **用户自定义系统提示词 / skill / MCP 接入** | 本期不做 | 确定的后续方向，但会显著推高复杂度，与 §1.1「先跑通」的定位冲突。§7.2.2 已预留角色与共享边界 | P0–P4 跑通，且教师提出实际需求 |
| **对接学校统一身份认证（CAS / OAuth）** | 不采用 | 自建账号体系不依赖信息化部门的对接排期，见 §7.2.1 | 用户范围扩大到全校，或学院要求账号统一 |
| **OAuth2 / JWT 认证** | 不采用 | 无第三方应用场景；Session 更简单且可主动失效，见 §7.2.3 | 出现第三方应用需代表用户访问本平台 API |
| **静态加密 / 服务间 mTLS / 发往 LLM 的数据脱敏** | 不采用 | §2.4 合规结论已解除这些要求，见 §7.4 | 合规要求变化，或平台开始承载敏感数据 |
| **HTTPS（自签证书）** | 不采用 | 无域名签不了受信证书；自签会让浏览器每次告警，代价大于内网明文的风险，见 §4.4 | 需要 secure context 的浏览器 API，或开放到内网之外 |
| **面向全院学生开放** | 本期不做 | 用户范围限定为教师 + 其课题组研究生（§2.2）。放开会使基数增一个数量级 | 学院明确要求扩大范围 —— 届时须重审 §3.2、§8.1 与 [ADR-0001](./adr/0001-single-host-compose.md) |
| **课题组内角色（组长 / 成员）** | 不采用 | 权限判定会从「查 role」变成「查 role × 查组内身份」，复杂度翻倍且当前无场景，见 §7.2.2 | 出现需要按组区分权限的实际需求 |
