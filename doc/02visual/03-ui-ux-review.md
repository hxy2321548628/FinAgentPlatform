...
| D2 | 暗色模式时机 | 维持"预留不实现" / 本期实现 | **决策变更（2026-08-16）**：原选"预留不实现"；P1 状态色 token 化与组件 class 化完成后前置条件满足，改本期实现（DSD §2.2.1），图表浅底为已知债务 |
...
| 2026-08-16 | **对话页十条反馈第一批**：助手头像改平台 Logo（消息流）；「分析思路」默认收起、流式时思考以单行预览内联标题行（仿 DSH Web）；删除用户气泡下的「本轮配置」冗余块；历史轮次滚进视口自动回放（IntersectionObserver 上下 600px、粘住不回退），不再逐条点击 | DSD §2.3 |
| 2026-08-16 | **对话页十条反馈第二批**：欢迎页懒创建 —— `/workspace/chat` 无会话时居中欢迎页、输入区可用、首次发送才建会话（提交成功才跳转，失败保草稿）；「新建分析」改纯导航（幂等，不留空会话）；会话行重命名/删除收进「⋮」菜单（Radix DropdownMenu，悬停/聚焦/打开时可见、触屏常显）；会话搜索（前端 300ms 防抖 + 后端 `GET /threads?q=` 标题模糊匹配） | DSD §2.1 |
| 2026-08-16 | **对话页十条反馈第三批**：配置会话级持久化 —— 完成/发送时把本轮配置写回 `thread.agent_config`（PATCH），重新打开会话自动回填表单（裸 ID 与冻结三元组快照都能还原），继承且无追加时不写；欢迎页懒创建路径在 Chat 提交时一并写入；kv cache 友好（会话默认稳定） | DSD §2.5 |
| 2026-08-16 | **输入区与配置弹窗重设计**：composer 组合框（圆角 14、聚焦描边、自适应高度、配置摘要胶囊 + 发送/停止）；配置弹窗改**标签页**（智能体/Skill/子智能体/MCP，徽标计数、方向键切换）、固定尺寸 560×560（换页不跳动）、挂载选项**固定高度内部滚动 + 每页 12 个分页加载**（配筛选框）、校验失败自动跳出错页签；工作台侧栏视觉刷新（分组小标题、药丸激活态、用户菜单 Radix 化） | DSD §2.5/§2.7/§2.1 |
