...
> **「input / output 两个单价」这个前提是错的**，实施时才发现：真实的 generation 报四个用量键，只定这两个会少算五倍。见 §8.10。
...
| MCP 审批放给 reviewer（§8.8c，计划外） | `P7⑤` 角色边界、`P10②` MCP 放行链路、两条后端用例、`AdminLayout` 那条前端用例 | 改实现的同时改掉记着旧决策的注释与用例；`P7⑤` 补三条断言钉住新的线 |
...
### 8.8 用户在真实使用里报出来的一处：总览与对话互相看空白

**症状**：在总览页刷新后进对话，侧边栏一片空白；对话页刷新后才出来，这时回总览，
最近会话又没了。

**真因是 query key 上混用了两种 hook。** `threadKeys.list()` 在 `ThreadSidebar` 与
`MyData` 里是 `useInfiniteQuery`（缓存形状 `{pages:[…]}`），而总览接线时我用了普通
`useQuery`（形状 `{items:…}`）。React Query 一个键只有一份缓存 —— **谁先加载谁的
形状占住它，另一边读到的字段全是 `undefined`，页面空白且不报错。**

**门禁抓不到它**：两个页面各自渲染都是对的，只有共用同一个 `QueryClient` 时才现形，
而单页测试正是各渲染各的。新加的 `Overview.test.tsx` 因此**先按 infinite 的形状把缓存
塞进去再渲染总览**。把实现改回 `useQuery` 验过一次，它确实会红。

> 它的形状值得记住：**两处代码各自都对，错的是它们共用的那个键。**

### 8.8b 第二处使用者报出来的：审核员进不去后台

**症状**：审核员打不开 `/admin`。

查下来**不是一个 bug 是三个**，任一单独修都不够 —— 合起来才让这个角色完全没有路进去：

| # | 缺陷 | 为什么单独看不出来 |
|---|---|---|
| 1 | 裸 `/admin` 的 index 挂在 `AdminGuard` 那一块 | React Router 按路由排名选分支，index 的得分压过 `ReviewerGuard` 那一块的 splat，于是 reviewer 打开 `/admin` 撞的是**管理员**守卫 → 404。而 `/admin/agents` 一直是通的 |
| 2 | 登录跳转只认 `admin` | reviewer 被送去工作台 |
| 3 | **工作台里一个后台入口都没有** | 连 admin 也没有 —— 它只靠登录那一跳进去，点了「返回工作台」就回不来 |

**既有的验证全都绕过了它。** `AdminGuard.test.tsx` 每条都过，因为它测的是「守卫放不放行」，
而坏的是「**哪个守卫接住了这个路径**」；`P7⑥` 的 playwright 走查里 reviewer 那一段是
`page.goto('/admin/agents')` —— 直接输 URL，正好跳过了入口缺失这一层。

修法连带把 `App` 的路由表拆成 `AppRoutes` 导出：新测试要跑**这一份**路由表，在测试里
另搭一份等于什么都没验。4 条新用例已验证「把 index 挪回去就红」。

### 8.8c MCP 审批的边界重划

使用者问「MCP 也可以审核吧」。原来的线划在**资源类型**上（agent / skill 归 reviewer，
MCP 只有 admin），理由写在 `route/mcp.py` 的模块 docstring 里。定案改成**按审核与运维分**：

- 批、拒、看队列 → reviewer 与 admin，与 agent、skill 同一档；
- 启停、探活 → 仍只有 admin。它们改的是一个**已放行**的服务此刻通不通，与「该不该放行」无关。

前端跟着走，且 reviewer 看不到那两个运维按钮 —— **摆一个必然失败的按钮比不摆更糟**。
`REVIEWABLE_KIND` 保持不含 MCP：它有自己那一页（带申请人、失败计数、探活结果），
两个入口同时列同一批申请只会让两边状态看起来不一致。

改了实现就改论证：三处记着旧决策的注释与两条测试一并重写。

### 8.9 收尾的三轮回归，与又一次「打穿历史判据」

**第一轮：P11 组根本没跑起来。** 脚本里写了 `$WORK` 而实际变量名是 `$WORK_DIR`，
`set -u` 下第一次引用就退出 —— 连结果表都没打印。顺带把 P11 组从脚本末尾移到 P10
之后：P0 与 P1 有意排在最后（最贵、最危险），P11 该按编号跟在 P10 后面。

**第二轮：`P5①` 被本期打穿。** `email` 变成必填之后，那三处走 `/api/auth/register`
的请求一律 422，整段红成「注册结果不对」—— 指向注册流程而不是「缺一个字段」。
§6 的回归关系预判到了「所有建号路径」，但上一轮只修了 `make_user()` 那条 SQL，
**漏了走 API 的这条**。

> **一个操作教训**：第二轮跑到一半时我去改了 `verify.sh`，于是它当场报语法错误 ——
> bash 是流式读脚本的，运行中改动会让解析错位。脚本本身没坏（`bash -n` 通过），
> 但那一轮的结果作废了。**跑着的时候不要碰它。**

**第三轮：`P5①` 仍红，且 `P11⑤` 红的是判据本身。** 两条都值得记：

- `P5①`：上一轮给注册补 `email` 时我用了 `@verify.local`，而 `.local` 是保留 TLD，
  `EmailStr` 一律拒绝。**SQL 造号那条路绕过校验所以看着没事**，走 API 注册的三处 422 ——
  同一个域名在两条路径上表现不同，正是它躲过上一轮的原因。改成 `verify.zuel.edu.cn`。
- `P11⑤`：**红的不是产品，是我写的判据。** 闸门判的是「今日已用 >= 上限」
  （`_require_quota`），新账号已用 0、上限设 1 时 `0 >= 1` 不成立，本来就该放行 ——
  那是设计如此，`policy.py` 里写明了「`0` 是一次都不许跑」那一档。判据改设 0。

  > 同时补了一步：**撤回配额后必须重新提交得上**。少了它，一个「无论如何都拒绝」的
  > 实现也能让这条绿 —— 与 §4.3 拒绝代理指标是同一个道理，只是方向相反：
  > 那里防的是「没真拦住」，这里防的是「拦错了东西」。放行那次收下的 run 立刻取消，
  > 这条判据不该花 DeepSeek 的钱。

### 8.10 模型价格：四个用量键，漏一个就少算五倍

`register-model-price.sh` 的第一版只注册 `inputPrice` / `outputPrice` 两个数。查 Langfuse
的 ClickHouse（v4 的 `events_only` 模式不写 `observations` 表，数据在 `events_core`，
这也是 `/api/public/observations` 404 的原因）拿到真实的一条：

```
usage_details: {'input':22, 'output':292, 'total':3934, 'input_cache_read':3584, 'output_reasoning':36}
```

四个键**互不重叠且加起来正好等于 `total`**。两处因此必须改：

1. **`input_cache_read` 占九成的量。** 只定 input / output 的话费用少算五倍多 ——
   而那仍是一个「像模像样」的数字，比 0 难发现得多。
2. **`total` 一定不能定价**，它与那四个重叠，定了就是每笔算两遍。

`inputPrice` / `outputPrice` 本身也已被 Langfuse 标为 deprecated，现在是 `pricingTiers`。

单价定为 2026-08-17 起的**峰时价**（峰时 9-12 与 14-18，正好覆盖教师上课办公的时段；
Langfuse 的单价表达不了分时段，宁可高估不要低估），单位改成「每百万 token 的人民币」——
照价目表原样抄，不用换算。**界面上的 `$` 要当人民币读**：按汇率换算的话汇率一动数字
就悄悄偏了，而那种偏差不报错。

实测核对（一次真实分析，注册价格之后）：

```
cost_details: {input:0.000189, output:0.000054, input_cache_read:0.0010752, total:0.0013182}
```

缓存命中那一项占了 82% 的费用，与上面的判断对得上。

> **注册成功不等于算得出费用。** 这个接口对着不认识的字段也回 200，而「注册过了但
> 一个键都没定上」与「没注册」在界面上是同一个 0。脚本因此回读 `id` 与已定价的键，
> 而最终核对靠的是**跑一次真的分析**再看 `cost_details` —— 历史 generation 不会重算。
