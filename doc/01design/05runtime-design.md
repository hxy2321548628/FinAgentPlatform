...
>
> **2026-08-18 推翻上面这半条：`execute` 补上条件拦截**（`app/agent/interrupt.py`）。
>
> **起因是一次真实缺陷，不是模型漂移。** 沙箱 backend 改成只实现异步那一半之后，DeepAgents 探「支不支持删除」用的是 `type(backend).delete is not BackendProtocol.delete` —— 它只看**同步**方法有没有被覆写，于是探测静默转 False，**`delete` 工具整个不再挂给模型**。agent 想删文件只剩在 `execute` 里跑 `rm` 一条路，而闸门只认 `delete`，人工审批就此整个失效：回归验收上 P3① 四种决策与 P9⑤ 嵌套审批五条断言同时红，而没有一条指向沙箱 backend。缺陷本身已在 `RemoteSandboxBackend` 修掉（同步 `delete` 覆写成一句显式抛错，专为让能力探测认得出它）。
>
> **修掉缺陷之后仍然加这道条件拦截，因为它暴露的是设计上的单点**：闸门只认一个工具名，任何让那个工具消失、或让模型换条路的变化，都能把整道闸静默绕过 —— 而绕过时没有任何东西会报错。`execute` 的条件拦截是纵深，不是替代。谓词只看 `command` 一个字符串，是纯函数，不破坏索引匹配。
>
> **那条观察项没有被跳过，量过了**：库里 112 次真实 `execute` 调用，谓词命中 6 次，而这 6 次**全部**来自「教师明确要求删文件」的 run —— 纯分析的那 106 次一次都没命中。既不是 0（太严）也不是十几次（没法用）。此后随每次回归验收再量一次（`verify.sh` 的 P0 观察项）。
>
> **谓词的边界写在模块 docstring 里**：它只认命令名位置上的 `rm` / `rmdir` / `unlink` / `shred`，`python -c "os.remove(...)"`、`find -delete`、`sh -c "rm x"` 一律绕得过去。这道闸管的是教师知不知情，不是删得掉删不掉 —— 破坏面本来就被沙箱圈在本会话的工作目录里。
>
> **同时补上提示词契约**：告诉 agent 删文件走 `delete`。闸门拦得住 `rm`，但每拦一次就多一轮审批往返 —— 契约省的是这一轮，不是防绕过的。
...
> **2026-08-18 发现：调用级这一层同样从未落地**，与下面那条 2026-08-09 的批注是同一笔账的另一半 —— 那条只记了 run 级。逐项核实的结果：
>
> - **LLM 调用**：确实会重试，但那是 OpenAI SDK 自带的默认值（`DEFAULT_MAX_RETRIES = 2`），不是本节设计的「1s → 2s → 4s，上限 3 次」，平台一行都没写；`ChatDeepSeek` 的 `max_retries` 留空，因此吃的就是 SDK 默认。
> - **broker 请求**：`sandbox/remote.py` 用的是裸 httpx，**一次都不重试**。
> - 顺带：Redis 客户端 `retry` 为 `None`，Postgres 只有 `pool_pre_ping`（连接前探活，不是重试）。
>
> 于是「**这是主要手段**，绝大部分瞬时故障在这一层就消化了」这句话目前没有实现支撑 —— broker 抖一下就是一次工具调用失败。**尚未裁决是改文档还是补实现**，与下面那条一并决定。
...
>
> **2026-08-09 发现：这一节从未落地，实现是「一次都不自动重试」。** `run/executor.py` 的模块 docstring 写的是「没有自动重试：失败时只在 `run.failed` 里给出 `retryable`，重不重试由人决定」，`runs` 表里也没有记重试次数的列 —— 也就是说本节设计的那 1 次从来没有实现过。**尚未裁决是改文档还是补实现**：两者都说得通（本节的理由是「幂等键没落地之前少重试一次少一次风险」，而 §5.6 的幂等键已于 P3 落地，因此放开重试的前提其实已经具备）。孤儿收割器（§8 运维那节）不依赖这个裁决 —— 它不重投的理由是收割器与 worker 两个进程各跑各的、重投会撞出双重执行，与重试几次无关。
