...
> **事件流单独记一本账**（2026-08-16 加，键前缀 `stream:`，与普通请求的 `user:` 分开）。
>
> 长连接靠重连维持，而**每次重连都要过一次这道闸**。共用一个计数器时它不只是多占名额，而是会**自持**：被拒的流按固定间隔重连，速率高过窗口清空的速度，闸门就再也开不了。症状是「点什么都是 429」，且没有一处指得出源头在一条流上——2026-08-16 实测踩过，教师侧表现为整个界面卡死在「操作太快了」。
>
> 分账之后两类流量互不影响，而**流那本账仍旧有闸**：不限的话，开无限条流就没有任何东西拦得住。前端配套改了两处，缺一不可——事件流重连改指数退避（1s→30s，此前是库的固定 1000ms），以及 **429 一律不重试**（此前 `RATE_LIMITED` 会自动重试两次，把一次误触发放大成三倍流量）。
