...
| 不做 | 后果 | 由哪期偿还 |
|---|---|---|
| MinIO 产物存储 | 产物仍从 workspace 直接读；workspace 仍不回收 | 本期定案移到 P3（§2.1），**2026-08-08 又退回 P4**（[P3 计划 §7.1](./P3-plan.md)） |
...
>
> 本节三条理由**没有一条被推翻**，但其中第二条被读得太重了：「隔离设计依赖 P3 的用户模型」说明它**不能早于** P3，不等于**必须在** P3。而本节第三条（「往最大的一期里再塞一个存储组件」）恰恰在 P3 身上重演 —— P3 有八个步骤，比 P2 还大。
>
> **这次的教训值得记一笔**：把一件事从自己身上挪走时，顺手指的那个期次容易变成既成事实。挪走的决定和落到哪一期的决定，是两个决定。
...
与 [P1 §4](./P1-plan.md) 同样的口径，**一条命令能跑完**。

```bash
make all                                     # 门禁全绿
docker compose -f deploy/compose.yml up -d   # nginx + api + worker×2 + broker + postgres + redis

bash deploy/test/verify.sh                             # 本期四条编号 P2①②③⑥，与其余各期一起跑
SKIP_LLM=1 bash deploy/test/verify.sh                  # ① 要真实分析，跳过则记「未验」
```

> **脚本在 2026-08-13 换了**：`p2.sh` 已并进 [`deploy/test/verify.sh`](../../deploy/test/verify.sh)，
> 转调没了 —— 本期的 ④（P0 回归）与 ⑤（P1 回归）原本只是「去跑另一个脚本」，
> 现在跑整个文件就把它们都跑了。判据编号原样保留；下方 §8 里的 `p2.sh` 是当时的记录。
...
### 7.1 幂等键：实测结果与据此的决定（2026-08-07 定案，2026-08-14 补两轮并更正读法，2026-08-16 补第五轮）

验收①跑了五轮崩溃恢复，脚本打印的观察项分别是：

| 轮次 | 日期 | 工具调用 | 工具返回 | 差值 |
|---|---|---|---|---|
| 第一轮 | 2026-08-07 | 4 | 4 | **0** |
| 第二轮 | 2026-08-07 | 5 | 5 | **0** |
| 第三轮（`verify.sh` 整合后重跑） | 2026-08-14 | 4 | 4 | **0** |
| 第四轮（全量回归，[P7 §8.6](./P7-plan.md)） | 2026-08-14 | 4 | 4 | **0** |
| 第五轮（全量回归，[P10 §8.6](./P10-plan.md)） | 2026-08-16 | 3 | 3 | **0** |

**五轮都是 0：没有观测到任何工具重复执行。**

> **前两轮的读法后来被推翻了（2026-08-14 回填时才发现）。** 当时写的是「`run.started` 只有
> 1 条，说明恢复走的确实是续跑而非重跑」—— 这条推断是错的。[P3 §8.5](./P3-plan.md) 之后
> 查明（[P3 §8.7](./P3-plan.md)）**重投照样会发一条 `run.started`**，因此 1 条恰恰意味着**那一刀根本没砍到**：run 在
> kill 生效之前就已经跑完了。按今天 `verify.sh` 的判据（`run.started ≥ 2` 才算真崩），
> 前两轮**什么都没量到**，样本数实际是 **2 而不是 4**（补进第五轮后是 **3 而不是 5**）。
>
> 第三、四、五轮的 `run.started` 都是 2 条，是这项观察真正的三个样本。
> **§7 那个「不提前做幂等键」的结论不受影响** —— 它本来就只靠「没量到」支撑，
> 而现在的证据比当初以为的还要薄一半。
...
- **只有三次真的砍到，且崩溃点都是随机的。** `kill -9` 挑在 run 进入 `running` 之后约 20 秒落下，砍到的那三刀都落在了两次工具之间。砍进 `execute` 执行途中的那一刀会是什么样，五轮都没有覆盖到。
...
**这不是遗漏而是取舍**：本期验收六条没有一条依赖它，而它要改 broker 的申请接口（带上持有者身份）与新增一个「按持有者回收」的端点，属于另一块独立的改动。

> **已于 2026-08-08 由 [P3 步骤七](./P3-plan.md) 偿还。** 池里记的从「几个人在用」改成「谁在用」，持有者取 run 标识 —— 崩溃恢复接着跑的是同一个 run，重复申请因此是幂等的，那个名额不再要等 30 分钟。超时兜底 `_expire_lease` 原样留着：精确回收靠的是「另一方回来认领同一个 run」，而那个前提本身也可能不成立。
