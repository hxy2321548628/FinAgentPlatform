...
| 项 | 值 |
|---|---|
| 文档状态 | **全部定案**（24 条，2026-08-19；A4 存储范围于 2026-08-21 由 ADR-0019 补定） |
| 当前版本 | v0.8 |
| 作者 | hxy |
| 日期 | 2026-08-21（原始定案 2026-08-19） |
| 下游文档 | [P13-plan.md](./P13-plan.md)（**已写就 2026-08-19**）· [P14-plan.md](./P14-plan.md)（**已写就 2026-08-19**）· [P15-plan.md](./P15-plan.md)（**已撰写 2026-08-21**，按 §6 的 D1 分期） |
...
| 版本 | 日期 | 修改人 | 说明 |
|---|---|---|---|
| v0.8 | 2026-08-21 | Claude | **补定 A4 的物理存储与生命周期**：记忆采用 thread workspace 内的受保护 `.memory/`，同一 thread 的 run 共享；不同 thread 不继承；thread purge 递归删除记忆，所有字节计入该 thread 的 5 GB XFS 配额。跨 thread 的 user/group memory 不在 P15 实施。详见 [ADR-0019](../01design/adr/0019-thread-workspace-memory.md) 与 [P15-plan §2.3](./P15-plan.md)。 |
...
> ⚠️ **v0.7 的原始假设已被 2026-08-21 的 A4 补定替代**：记忆不再做跨 thread 或同组共享，而是放在当前 thread 的受保护 `/workspace/.memory`；同一 thread 的后续 run 可召回，thread purge 递归删除并释放其 workspace 配额。教师只能在当前 thread 查看/删除，管理员与其他用户统一 404。实现边界见 [ADR-0019](../01design/adr/0019-thread-workspace-memory.md)。
...
| 跨 thread、跨用户或跨组共享记忆 | A4 补定为 thread-private；共享会重新引入串味、权限和保留期问题 | 另开 ADR 后重估 |
...
| **P15** | **P13 量出的噪声带**（D5：方差压不住则整份验收标准改写）+ **A4 的 thread-private `.memory` 定案**（ADR-0019：同 thread 跨 run、删除 thread 连带清理、计入 5 GB 配额）+ **P14 之后的新成本基线**（下面三条 2026-08-19 由 P14 回写） |
