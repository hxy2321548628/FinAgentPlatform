...
def test_registering_with_an_invite_code_joins_that_group(client: TestClient, platform: Platform) -> None:
    ...
    response = client.post(
        REGISTER_PATH, json={"name": name, "password": NEW_PASSWORD, "invite_code": group.invite_code}
    )
...
def test_someone_who_used_an_invite_code_can_log_in_at_once(client: TestClient, platform: Platform) -> None:
    ...
    group = make_group(client, platform, owner_id=_owner_id(client))
    name = _name()
    client.post(REGISTER_PATH, json={"name": name, "password": NEW_PASSWORD, "invite_code": group.invite_code})
...
def test_someone_who_used_an_invite_code_is_on_the_roster(client: TestClient, platform: Platform) -> None:
    group = make_group(client, platform, owner_id=_owner_id(client))
    client.post(REGISTER_PATH, json={"name": _name(), "password": NEW_PASSWORD, "invite_code": group.invite_code})
...
def test_registering_without_a_code_cannot_log_in_yet(client: TestClient) -> None:
    ...
    name = _name()
    response = client.post(REGISTER_PATH, json={"name": name, "password": NEW_PASSWORD})
...
def test_a_wrong_invite_code_is_refused(client: TestClient) -> None:
    response = client.post(REGISTER_PATH, json={"name": _name(), "password": NEW_PASSWORD, "invite_code": "NOSUCH99"})
...
def test_a_wrong_invite_code_leaves_no_account_behind(client: TestClient, platform: Platform) -> None:
    ...
    name = _name()
    client.post(REGISTER_PATH, json={"name": name, "password": NEW_PASSWORD, "invite_code": "NOSUCH99"})
    assert client.portal is not None
...
def test_a_taken_name_is_refused(client: TestClient) -> None:
    name = _name()
    client.post(REGISTER_PATH, json={"name": name, "password": NEW_PASSWORD})
...
def test_a_registered_account_is_always_a_student(client: TestClient) -> None:
    ...
    response = client.post(
        REGISTER_PATH, json={"name": _name(), "password": NEW_PASSWORD, "role": UserRole.TEACHER.value}
    )
...
def test_registering_does_not_hand_out_a_session(client: TestClient, platform: Platform) -> None:
    ...
    response = client.post(
        REGISTER_PATH, json={"name": _name(), "password": NEW_PASSWORD, "invite_code": group.invite_code}
    )
...
def test_registering_is_open_to_anonymous_callers(client: TestClient) -> None:
    ...
    assert client.post(REGISTER_PATH, json={"name": _name(), "password": NEW_PASSWORD}).status_code == 200


def test_the_password_never_shows_up_in_the_log(client: TestClient) -> None:
    with json_log("api.route.auth") as recorded:
        client.post(REGISTER_PATH, json={"name": _name(), "password": NEW_PASSWORD})
        client.post(REGISTER_PATH, json={"name": _name(), "password": TEST_PASSWORD, "invite_code": "NOSUCH99"})
