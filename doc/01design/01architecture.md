...
| v0.15 | 2026-08-08 | hxy | 依 [P3 计划](../03plan/P3-plan.md) 的六项定案回填，**关掉四个悬了很久的 TODO**：§5.3 审批范围（只全量拦 `delete`）、§6.4 配额数值（给保守初值；并发现原文与 §5.7 的 message 在「超额行为」上一直矛盾，以 §5.7 为准）、§7.5 限流阈值（宁松勿紧）、§6.2 skill 表预留（不预留，并把同一条理由推到 `groups` 上 —— 两张表本期都不建，`/auth/me` 因此不返回所属组）。**MinIO 二次改期，落定 P4**（§4.4 §6.5 §11）；§11 的 P3 一行补上租约记名与计划链接 |
...
| Agent 开发者 | §5.5、§5.6 + [智能体设计](./03agent-design.md) | 工具怎么定义？文件落在哪？上下文怎么管？ |
...
> **P2 已落地 `nginx` / `api` / `worker` / `sandbox-broker` / `postgres` / `redis` 六个**，见 [`deploy/compose.yml`](../../deploy/compose.yml)。只剩 `minio` 未落地：它 2026-08-07 从 P2 改到 P3，**2026-08-08 按 [P3 计划 §7.1](../03plan/P3-plan.md) 定案退回 P4**（理由见 §6.5）。
>
...
**P4 起多了可观测性五件套，实测占 0.41 GB**（2026-08-08，见 [P4 计划 §8.8](../03plan/P4-plan.md)）：Prometheus 58 MiB、Grafana 160 MiB、Tempo 91 MiB、Loki 71 MiB、OTel Collector 36 MiB。**不足以改上面这个推算** —— 0.41 GB 落在「约 14 GB」那一项的取整误差里，连四分之一个沙箱名额都占不到。

> P4 计划 §1.1 立项时按 2–3 GB 估算（约 1.5 个沙箱名额），据此论证「6% 的容量换完整排障能力划算」。**实测小了 5–7 倍**，那笔账因此更划算，但**论证方式仍然是对的** —— 当时手上没有实测值，按估算的上界决策是正确做法。

**同一次实测顺带发现：空闲沙箱的实际内存占用远低于 2 GB 上限**（两个沙箱合计 36 MiB）。**这不改上面的除法** —— 限额存在的意义正是挡住跑 pandas 的那一类，而不是刚起来的那一类。要按实际占用重算容量，得先攒真实负载下的样本，`zuel_sandbox_memory_byte` 这个指标就是为此而设。
...
> **MinIO 的期次两次改动，2026-08-08 落定在 P4。** P2 那次把它挪出自己身上时顺手指了 P3；写 P3 计划时才发现驱动理由是「租户前缀隔离依赖 P3 的用户模型」—— 那说明它**不能早于** P3，不等于**必须在** P3。而 P3 已有八个步骤，是四期里最大的一期。落回 P4 也与本文 §11 的 P4 一行「产物存储完善」一致，两处不再打架。
...
**重估条件**（任一成立即重开这一问）：单会话产物的典型值涨到 100 MB 量级（例如开始产出大数据集而不只是图表）；或用户基数从「课题组研究生」放开到全院（§2.2 的边界，§10.2 已把它列为风险）。
...
- **没有「保留 N 年后自动删除」这条规则。** 定它需要学校的数据管理规定，而 §2.4 的结论是本平台不处理敏感数据、无合规硬约束 —— 没有约束就没有依据，凭感觉定一个年限只是把随意写进文档。**触发条件**：学校出台相关规定，或磁盘占用真的成为约束（见上一节的重估条件）。
...
| **workspace 本地副本无界增长** | 配额是 per-thread 的，而 workspace 在容器销毁后仍留在宿主机。总量 = 历史 thread 数 × 5GB，worst case 是 TB 级 | §6.5 保留策略待定。**扩容只是推迟撞墙**，必须做归档回收 —— [P4 计划 §2.3](../03plan/P4-plan.md) 定案本期不做（实测收益与复杂度不匹配），**仍无期次** |
...
| **P3** | HITL 审批 + 取消 + 多用户隔离 + 配额限流（含 §6.4 的 cache 拆分口径）+ **§5.6 工具幂等键**（已与 HITL 解耦，见下）+ 沙箱租约记名（P1 遗债） | 审批流程走通；`kill -9` 在工具执行途中，恢复后写操作不重复执行；30 并发压测不崩溃、不串数据。步骤分解见 [P3 实施计划](../03plan/P3-plan.md) |
| **P4** | 可观测性（OpenTelemetry）、**MinIO 产物存储**、成本看板 | 能定位单个 run 的完整 trace 与 token 花费 |
...
| 第三方接入规范 | [`doc/01design/04extension-integration.md`](./04extension-integration.md) | 下游，把附录 B「自定义提示词 / skill / MCP」一行展开为对外规范。**平台侧实现未排期** |
...
| **用户自定义系统提示词 / skill / MCP 接入** | 本期不做 | 确定的后续方向，但会显著推高复杂度，与 §1.1「先跑通」的定位冲突。§7.2.1 已预留角色与共享边界。**对外规范已先行制定**，见[第三方接入规范](./04extension-integration.md) | P0–P4 跑通，且教师提出实际需求。**2026-08-03：后半条已触发**（有教师提出接入需求），前半条未满足，故仍不做 |
