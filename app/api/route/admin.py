...
**成本这一份只有端点，没有页面。** 曾经配过一张自包含的静态页，理由是「一个没人看的
JSON 端点落不了地」；那一页已经撤掉，眼下由 Grafana 的「今日未命中 token（按用户）」
顶着日常巡检。**两者不等价** —— 那个 panel 读的是每天归零的 gauge，
而这个端点直接查 `runs` 表，任意时间窗都答得出。要问「上个月谁花了多少」只能走它。
"""
