...
| 项 | 内容 |
|---|---|
| 产出 | 事件日志（内存实现）+ 驱动 agent 消费流、写事件、维护 run 状态 |
| 依据 | [架构 §5.1](../01design/01architecture.md) 时序、[§5.4](../01design/01architecture.md) 状态机（P0 只用四态） |
| 验证 | ① 跑通验收 case；② 事件序列完整、`id` 单调递增；③ **能从任意 `id` 之后重放**，这是步骤四断线重连的前提 |
...
| 项 | 状态 |
|---|---|
| **P0 是否连最小前端一起做** | **待定**。三个选项：只做后端用 curl 验收 / 加一个无样式单页 / 按[前端选型](../01design/02frontend-selection.md)搭正式 React 工程。倾向第一个 —— 最快拿到「agent 到底好不好用」的结论（[架构 §10.2](../01design/01architecture.md) 风险二），且事件契约在真实流量下跑过一轮再动前端，可避免契约微调时前后端一起返工 |
