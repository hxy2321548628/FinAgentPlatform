...
| 选项 | 后果 |
|---|---|
| a. 自填 URL / command | 教师可以往 worker 进程（信任区、可出网、持有 DeepSeek key）里注入任意连接；stdio 型 MCP 直接是 worker 容器内任意代码执行 |
...
> **⚠️ 2026-08-15 更正（P10 开工前实测，定案不变、理由改了）。** 本条原来给选项 a 记的第一条
> 后果是「§3.1 的后果：一次同步阻塞卡住整个事件循环，**你的服务慢 30 秒，全平台所有教师同时
> 卡 30 秒**」。**这句对 SSE / streamable-http 不成立**：`langchain-mcp-adapters` 生成的是
> 只有 `coroutine`、`func=None` 的 `StructuredTool`，**只能异步调用** —— 一次慢调用挂住的是
> 那一个 run 的那一步，worker 的事件循环照常调度别的 run（[P10 §2.5](./P10-plan.md)）。
>
> **定案不变**，不能自填的理由是另外两条，且都成立：stdio 型 MCP 是 worker 容器内任意代码
> 执行（F2 因此禁掉 stdio）；教师能让 worker 进程（信任区、可出网、持有 DeepSeek key）向任意
> 地址发起连接。
>
> **这处更正必须写下来，因为那句话是 F5 取超时阈值时的主要依据。** 照着「卡全平台」去想，
> 30 秒会显得太长、恨不得设成 3 秒；而真实情况是「慢的只是这一次分析」，30 秒是合适的 ——
> 一个检索类外部服务偶尔慢十几秒很正常。**理由错了而结论对，下一次改这个数的人就会改错。**
...
> **⚠️ 2026-08-15 更正（P10 开工前实测）：30 这个数没错，错的是它落在哪里。**
>
> 本条原来的意思是「给 MCP 连接配上 30 秒超时」。实测一台**接受 TCP 连接但永不回应**的
> 服务：给 connection 传 `timeout=2.0` 也要 **300.05 秒**才抛出来，什么都不传则 12 秒时
> 仍未返回。源码里的默认值解释了这件事 —— `DEFAULT_STREAMABLE_HTTP_TIMEOUT = 30s` 管连接
> 与写，而 `DEFAULT_STREAMABLE_HTTP_SSE_READ_TIMEOUT = 300s` 管读等待，**真正决定「一个不
> 回应的服务能挂住你多久」的是后者**。教师看到的是「点了提问，什么都没发生」，日志里那五
> 分钟一片空白。
>
> **P10 的落地**：超时由平台自己 `asyncio.timeout` 压住，装配（`get_tools`）与单次调用
> （interceptor）两处都包；connection 的两个字段也一并设成平台的值作双保险，**但判据盯的是
> 外层那道**（[P10 §2.2](./P10-plan.md)）。**这条防线不是框架给的，是平台自己包上去的** ——
> 因此 `P10③` 是端到端判据，断言的是真实耗时，不是「我们传了这个参数」。
>
> **另外两条也在 P10 落地时定了**：熔断**只数传输层失败与超时**，工具自己报错（`isError`）
> 不计 —— 模型传错参数在一次分析里连着发生 5 次很常见，那会停掉一个健康服务；三条路
> （装配、工具调用、管理员探活）**共用同一个计数器**，否则验收判据验的是一条没人走的路。
...
> **⚠️ 2026-08-15 更正（P10 开工前实测）：撞名的后果比上面写的更糟，它不是整洁性问题而是安全问题。**
>
> 上面判断撞名的后果是「两个同名工具，LLM 调哪个由框架的注册顺序决定」。**实测不是这样**：
> 把一个提供 `read_file` 的外部 server 交给 `create_deep_agent(tools=…)` 之后，模型这一轮被
> bind 到的工具列表里**只出现一个 `read_file`，而赢的是外部那个** —— 平台内置的被静默丢掉，
> 没有任何警告（[P10 §2.1](./P10-plan.md)）。
>
> 于是**一个上架时人畜无害的服务，只要事后加一个叫 `read_file` 的工具，就能接管平台的文件
> 读取** —— agent 以为在读沙箱，实际把路径与后续内容发去了校外。
>
> **P10 的落地**：防线从上架审核挪到**装配层** —— 拿到外部工具后先剔除与内置 8 个及 `task`
> 撞名的，记 WARNING，再交给 `create_deep_agent`。上架时的撞名检查照做，**但它挡不住上架
> 之后的变化**：那份清单会过期，装配时的剔除不会。清单比对相应改成**只记 warn 不跳过整个
> server** —— 拦下来会让一次正常的上游升级打断所有人的分析，而真正危险的那几个工具已经在
> 装配层被剔掉了。
>
> **为什么仍然不改成加前缀**（F6 的被否方案）：那是另一件事。加前缀能解决撞名，但会让所有
> 历史 run 快照里的工具名对不上，而本条定案的理由（LLM 读起来自然）仍然成立。**剔除是更窄
> 的动作**：它只在真撞上时生效，而真撞上时那个外部工具本来就不该被调用。
...
| **P10** | ~~**上架审核标准要重写** —— F0 指出接入规范 §5.4 的 13 项交付物在全外网前提下只剩 4 项成立~~ **已于 2026-08-13 随上游文档修订一起做掉**；P10 开工时（2026-08-15）只需按九处实测把 §5.4 第 5、6 项的措辞改准（比对不一致是记 warn 不是跳过整个 server；30 秒那道超时是平台自己包的） |
