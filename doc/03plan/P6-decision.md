...
>
> **定案汇总**：字节存宿主机目录（broker 持有）；ZIP 与单 md 都收；校验按下方七条阈值；**每次 run 开跑前全量对齐**；物化到明文 `/workspace/skill/`，用户看得见；agent 的写权限完全放开；不单独管配额；同会话重名在配置时拦住；展示名只用 frontmatter；不加机器检查；不做 `allowed_tools`；不预置平台 skill。
...
前端 `MySkills` 现在只画了 ZIP（「整个 Skill 目录的 ZIP，根目录必须包含唯一的 SKILL.md」），**要加一条单文件路径**：直传的 `.md` 由后端自动包装成一个只含 `SKILL.md` 的目录。
...
| 压缩比上限 | 100:1 | 第一道 |
| 单文件大小 | 1 MB | |
| 允许扩展名 | `.md` `.txt` `.py` `.csv` `.json` `.yaml` `.toml` | 白名单而非黑名单 |
| 符号链接 | 一律拒绝 | |
| 路径穿越（`..`、绝对路径） | 一律拒绝 | |
...
### D10 展示名以谁为准 —— **定案：只用 frontmatter 那份**

**与建议相反（我建议存两份）。** 采纳的是「一份数据不会不同步」，与前端 `MySkills` 现有文案完全一致（「平台将从 SKILL.md 的 frontmatter 读取名称与描述」），前端不用改。
...
### G6 事件的嵌套怎么渲染 —— **定案：后端先补真实子图用例，前端做可展开折叠块**

`EventEnvelope.path` 早就预留了，但 `_map_path`（`event/mapper.py:186-194`）的注释明写「实测 ns 全程为空，**这条剥离逻辑没有真实子图样本验证过**」。前端 `MessageList` 也没有嵌套分支。

**用例必须是真跑一次拿到的 chunk，不能是构造的** —— 构造的样本只能证明代码按我以为的形状工作，证明不了框架真的吐那个形状。这正是[「先验证验证代码」](../../CLAUDE.md)那条记忆的形状。

**这是本期唯一一处「事件契约第一次被真实数据填上」的地方，p6.sh 里必须单列一条判据。**
...
### G9 `RECURSION_LIMIT = 60` 够不够 —— **定案：每个 agent 单独算轮数**

主 agent 与每个子智能体各有自己的递归预算，不共用主图那 60。

> **⚠️ 这条依赖一个未经验证的框架行为**：LangGraph 的 `recursion_limit` 来自 `config`，而子图作为节点执行时**是各自计数还是共享父图的计数，没有实测过**。若实际是共享的，这条定案落不了地。
>
> **实施第一步就要验它**：跑一个会超限的子智能体，看抛出的是子图的限制还是父图的。共享的话退回「按子智能体数量动态算」（`60 + 40 × 子agent数`，那个 40 同样需要实测支撑）。

跑飞的表现是 run 失败并带 `INTERNAL` —— **看得见，不是静默失败**，这一点让本条的风险可接受。
...
### H2 会话中途改配置，已有 checkpoint 怎么办 —— **定案：改了下一次 run 就生效。已核实，不需要实测**

原稿把这一条列为「必须实测的未知」。**2026-08-13 已核实源码，结论是现有架构恰好已经支持配置热更新**：

| 依据 | 位置 | 含义 |
|---|---|---|
| `messages = [request.system_message, *messages]` | `langchain/agents/factory.py:1416` | system message 在**每次模型请求时动态 prepend**，**不进 state，因此也不进 checkpoint** |
| `_graph()` 每次调用都 `create_deep_agent(...)` | `app/agent/factory.py:164` | 图每次重建（原因是 backend 按会话绑定），因此 skill 与 subagents 也跟着按新配置重新装配 |

**三样都热更新**：提示词（动态 prepend）、skill（`SkillsMiddleware` 在建图时读 backend，而 D4 又在开跑前对齐过文件）、子智能体（建图时传入）。历史对话仍留在 checkpoint 里，不受影响 —— 这正是想要的行为。

> **⚠️ 2026-08-14 更正：上面这段里 skill 那一项是错的，实测推翻。**
>
> **定案本身不变**（「改了下一次 run 就生效」仍是要的行为），错的是「现有架构恰好已经支持」这个理由 —— skill 那一半**要平台自己做出来**，不做就是一个「配了没反应」的静默故障。
>
> `SkillsMiddleware` **不在建图时读 backend**，它在 `before_agent` 里读，且第一行就是「state 里已有 `skills_metadata` 就跳过（哪怕是空列表）」。而那个键标的是 `OmitFromSchema` 而非 `EphemeralValue`，**照常进 checkpoint** —— 于是同一个会话的第二次 run 根本不会重扫 skill 目录，哪怕图整个重建、文件已经对齐好了。
>
> 三次探针的数据与本期定案（子类化中间件让开那道短路）见 [P8 计划 §2.1](./P8-plan.md)。
>
> **这条更正对 H2 本身的教训**：「核实源码」核到的是**建图那一段**，而中间件的加载发生在**跑图那一段** —— 两段之间隔着一个 checkpoint。[「先验证验证代码」](../../CLAUDE.md)那条记忆在这里应验了一次：H2 当时的结论明写着「已核实，不需要实测」，而它恰恰是本组唯一被实测推翻的一条。
>
> **P9 要连带补一条**：子智能体（`SubAgentMiddleware`）有没有同形的短路，至今没有人验过 —— G6–G9 那四处实测里没有这一项。

**仍要在验收里带一条确认**：改一条显眼指令（如「所有回答以『喵』开头」），改完再问一句看它听不听。**核实源码不等于验过行为** —— 这是[「先验证验证代码」](../../CLAUDE.md)那条记忆的直接应用。
...
| **P8** | skill 全链路：上传 → 七条校验 → 宿主机存储 → 开跑前全量对齐 → `SkillsMiddleware` 接上 | **挂与不挂同一个 skill，产物里看得出差别** |
