...
Logo 沿用 DSD §6 的三横线图标（宽度递减的报告象形），仅替换文字部分。品牌名保持英文，与 DSD 的终端美学（等宽字体、全大写标签）气质一致。
...
| **RBAC 角色体系** | [架构文档 §351](../doc/01design/01architecture.md#L351) 的 `# RBAC角色` 是空标题。管理后台的页面量与权限粒度无法估算，`features/admin/` 暂缓展开 |
