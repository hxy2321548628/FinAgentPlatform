...
六个步骤已全部落地，每步一个提交。**验收六条中三条已过、三条待跑** —— 待跑的三条都要真实调用 DeepSeek，见 §8.4。
...
三个都是 437 条单测全绿、`docker compose up` 之后才冒出来的，已随 [`p2.sh`](../../deploy/test/p2.sh) 一并修复：
...
```bash
SKIP_LLM=1 SKIP_P1=1 bash deploy/test/p2.sh   # 已跑，②③⑥ 全过，约 1 分钟
bash deploy/test/p2.sh                        # 待跑，①④⑤ 要真实调用 DeepSeek
```

| 条目 | 状态 |
|---|---|
| ① `kill -9` worker 后从 checkpoint 续跑 | **待跑**（要 LLM） |
| ② 三进程重启后 run 终态与事件流都还在 | ✅ 已过 |
| ③ 两个消费者并行不重复也不丢 | ✅ 已过 |
| ④ P0 验收四条经新架构全过 | **待跑**（要 LLM） |
| ⑤ P1 验收六条不回归 | **待跑**（转调 `p1.sh`，其中②要 LLM、①要 sudo） |
| ⑥ 事件 id 契约不变 | ✅ 已过 |
