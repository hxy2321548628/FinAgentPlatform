      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 32 }}>
            {[
              { num: 'DeepSeek', label: 'POWERED BY' },
              { num: 'SSE',      label: 'STREAM' },
              { num: 'gVisor',   label: 'SANDBOX' },
            ].map(s => (
              <div key={s.label}>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#fff', fontFamily: "'JetBrains Mono', monospace" }}>{s.num}</div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', letterSpacing: '0.1em', marginTop: 3 }}>{s.label}</div>
              </div>
            ))}
          </div>
        ...
        <div style={{ flex: 1, padding: '60px 52px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          {tab === 'login' && (
            <form onSubmit={handleLogin}>
              <Field label="邮箱地址">
                <input className="login-input" type="email" placeholder="your@company.com" defaultValue="analyst@fin.edu.cn"
                  style={inputStyle} />
              </Field>
              <Field label="密码">
                <input className="login-input" type="password" placeholder="请输入密码" defaultValue="••••••••"
                  style={inputStyle} />
              </Field>
