...
def test_an_admin_can_create_a_teacher(client: TestClient, admin: str) -> None:
    ...
    response = client.post(USER_PATH, json={"name": _name("教师"), "password": NEW_PASSWORD, "role": "teacher"})
...
def test_the_created_account_can_log_in(client: TestClient, admin: str) -> None:
    ...
    as_admin(client, admin)
    name = _name("教师")
    client.post(USER_PATH, json={"name": name, "password": NEW_PASSWORD, "role": "teacher"})
...
def test_a_teacher_cannot_create_accounts(client: TestClient) -> None:
    ...
    response = client.post(USER_PATH, json={"name": _name(), "password": NEW_PASSWORD, "role": "teacher"})
...
def test_a_taken_name_is_refused(client: TestClient, admin: str) -> None:
    as_admin(client, admin)
    name = _name()
    client.post(USER_PATH, json={"name": name, "password": NEW_PASSWORD, "role": "student"})

    response = client.post(USER_PATH, json={"name": name, "password": NEW_PASSWORD, "role": "student"})
...
def test_activating_lets_the_waiting_student_log_in(client: TestClient, admin: str) -> None:
    ...
    name = _name("待激活")
    created = client.post(REGISTER_PATH, json={"name": name, "password": NEW_PASSWORD}).json()
    as_admin(client, admin)
...
def test_the_new_group_takes_students_in_by_its_invite_code(client: TestClient, admin: str) -> None:
    ...
    response = client.post(REGISTER_PATH, json={"name": _name("学生"), "password": NEW_PASSWORD, "invite_code": code})
