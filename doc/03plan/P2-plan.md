...
| v0.4 | 2026-08-08 | hxy | **验收六条全过，本期完成**。首轮⑤ 报红（拆 worker 后 api 日志丢了 run 身份），修复后重跑转绿。§7.1 补第二个观测样本（仍是 0），§8.3 增第五个问题（测试库号被 URL 静默盖掉） |
...
验收①跑了两轮完整的崩溃恢复，`p2.sh` 打印的观察项分别是：
...
**两轮都是 0：没有观测到任何工具重复执行。** 两次里 `run.started` 都只有 1 条，说明恢复走的确实是续跑而非重跑。
...
六个步骤已全部落地，每步一个提交。**验收六条全过**（2026-08-08）：五条一次通过，⑤ 首轮报红、修复后重跑转绿，见 §8.4。
...
| ③ 两个消费者并行不重复也不丢 | ✅ 已过 |
| ④ P0 验收四条经新架构全过 | ✅ 已过 |
| ⑤ P1 验收六条不回归 | ✅ 已过。首轮**未过** —— P1⑥ 报红，即 §8.3 第四行那个回归；修复后重跑，P1 六条全过 |
| ⑥ 事件 id 契约不变 | ✅ 已过 |

**P1 的六条在修复后的构建上完整跑过两遍**（一遍随 `p2.sh` ⑤ 转调，一遍单独跑 `p1.sh`），两遍都是六条全过 —— 包括要 sudo 的破坏性测试与要 LLM 的 P0 回归。**P2 到此可以宣告完成。**
| ⑥ 事件 id 契约不变 | ✅ 已过 |
