...
| 项 | 值 |
|---|---|
| 文档状态 | **开发完成，两条主判据都成立，全量回归 54 条全过**（2026-08-16）。实施记录见 [§8](#8-实施记录) |
| 当前版本 | v0.3 |
| 作者 | hxy |
| 日期 | 2026-08-15 |
| 一句话验收 | **外网工具真被调用** |
...
| v0.2 | 2026-08-16 | hxy | 开发完成，补 §8 实施记录。九个步骤逐步提交，判据 47 → 54。**§2 那九处实测全部落进代码且各有判据守着**（§8.2）。**部署之后又冒出两处门禁看不见的**（§8.3：一次 run 装配两遍、api 缺 `extra_hosts`），**验收脚本自己还骗过一次**（§8.4：端口被占，而红的地方指向熔断）。**§7 第 4 条那个方向性判断被实测推翻**：挂 MCP 反而多烧 token |
| v0.3 | 2026-08-16 | hxy | 补第二轮全量回归：**54 条全过，退出码 0**（§8.6）。`P1①` 那组破坏性测试在有 tty 的终端里补验通过，本期不再有未验条目。token 对照补到三组，方向一致且贵在 uncached（§8.5） |
...
| 4 | ~~**`ANALYSIS_TOKEN` 的校准**：样本会攒到 20 个。**MCP 与子智能体不同 —— 它不放大 token，反而可能减少**（工具直接给答案，模型不必自己算）。这个方向性判断要用 `P10①` 的两个样本验一次~~ **方向判反了**（2026-08-16 实测，[§8.5](#85-观察项与本期量到的数)）：不挂 `7424｜111｜223`，挂上 `11648｜411｜304` —— 挂 MCP **多**烧 token，因为工具 schema 要进每一轮 prompt，而「工具直接给答案」省下的那点抵不过它。**校准时要按「挂了 MCP 的 run 更贵」来算**，与子智能体同向而不是反向 | 样本够了单独一次改 |
...
九个步骤逐步提交，与计划 §5 的分解一一对应：

| 步骤 | 内容 | 提交 | 验证记录 |
|---|---|---|---|
| 一 | 夹具 MCP server 与四条离线实测 | `43acda0` | 后端 1155 → 1159 全绿；业务代码一行没动 |
| 二 | 数据层与迁移 `0014` | `170f7c7` | `alembic upgrade` / `downgrade` / `upgrade` 各一次；后端 1170 |
| 三 | 装配层 `agent/mcp.py` | `e535000` | 后端 1187；`P10③` `P10④` 的单测形态在这一步就跑得通 |
| 四 | 熔断 `agent/circuit.py` | `d85d1c9` | 后端 1196；含「三个入口同一个计数器」那条 |
| 五 | 目录、申请与审批 | `14f267f` | 后端 1223；`P10②` 的每条闸门都验两侧 |
| 六 | 主判据端到端 | `447ad2f` | 真实分析两次，双向断言成立；**修掉两处只有部署后才冒出来的问题** |
| 七 | 前端五处 | `26568c5` | 前端 132 → 142，双端门禁全绿 |
| 八 | playwright 第四条链路 | `088f4a3` | 两条链路通过，**且去掉 F12 缓解之后第二条真的红** |
| 九 | 验收脚本与文档 | `f38a4b3` + 本次 | 判据 47 → 54；免费的五条一次跑通 |

### 8.2 §2 那九处实测在代码里的落点

九处实测**全部在开工前跑完**，本期没有一处定案是「照文档写的」。落点：

| 实测 | 落点 | 守它的判据 |
|---|---|---|
| 外部工具静默顶掉内置 `read_file` | `agent/mcp.py` 的 `RESERVED_TOOL_NAME` 剔除 | `P10④` + 一条拿真实 `FilesystemMiddleware` 核对硬编码清单的单测 |
| 30 秒的落点错了（实测 300.05 秒） | `asyncio.timeout(MCP_CONNECT_TIMEOUT)` 包在 `get_tools` 外 | `P10③`（断言真实耗时 30.06 秒） |
| 超时一抛就掀桌 | `_CallGuard` 返回 `ToolMessage(status="error")` | `test_a_slow_call_comes_back_as_an_error_result_instead_of_raising` |
| 一个坏服务拖垮整批 | 逐个 server 各要各的 | `P10③` 第二条断言 |
| F1「全平台卡 30 秒」不成立 | 只改文档（P6 决策 F1 的更正） | `test_the_adapter_only_produces_async_tools` |
| 事件映射一行不用改 | `event/mapper.py` 确实一行没动 | `MessageList` 未知工具那两条 |
| `asyncio.timeout` 掐得干净 | 同上 | `P10③` 第三条断言 |
| 真外网连得通 | `P10⑦` | 同上 |
| 工具报错不抛异常 | 熔断只数传输层 | `test_a_tool_reporting_its_own_failure_comes_back_as_content` |

### 8.3 部署之后才冒出来的两处（步骤六）

**门禁全绿、单测全绿，两处都是 compose 起起来跑完一次真实分析才看见的。**

1. **一次 run 装配了两遍，MCP 因此连了两次**（日志里两条装配相隔 8 秒）。根因是 run 流跑完
   要查一次中断，而 `Agent.pending()` 走的是同一个 `_graph()`。中断记在 checkpoint 里，读它
   与图上绑了哪些工具无关 —— 那一步因此不再装外部工具（`_graph(with_mcp=False)`）。
   **不修的话每条挂 MCP 的 run 都多一次外网往返，且每一次都可能失败、都会记进熔断计数**，
   5 次的阈值实际只要两三条 run 就撞得上。§1.4 第 7 条预判了「多一次往返」，**没预判到是两次**。
2. **api 容器缺 `extra_hosts`**，管理员的「测试连接」连不上跑在宿主机上的夹具。真实的 MCP 全在
   校外用不上这一行，用得上它的是验收 —— 与 worker 那一行同理。

### 8.4 验收脚本自己骗过一次（步骤九）

`P10⑤` 第一次跑就红在「没自动停用」上。**真因与熔断毫无关系**：开发时手工起的一个夹具占着
8931，`verify.sh` 自己那个绑不上端口当场退出，而就绪探测探到的是**别人那一个** —— 于是一路绿到
`P10⑤`，那条要把夹具停掉，而 `kill` 一个早就死了的 PID 什么也不会发生，探活照常连得上。
**没有一处指向端口被占。**

修法写进脚本：起夹具前先确认端口没人应答、起完确认那个 405 是自己这个进程答的、停完等到端口
真的空出来。**这是「先验证验证代码」在本期的第二个落点**（第一个是 §4.4 那条「三个入口同一个
计数器」的单测）。
...
| 观察项 | 计划里的预期 | 实测 |
|---|---|---|
| `isError` 出现几次（§1.4 第 5 条） | 长期为 0 则熔断口径可以收紧 | **两个样本都是 0 次**（收尾两轮回归各一个） |
| 装配多花多少（§1.4 第 7 条） | 本地夹具 39 ms、真外网 3.3 秒 | 本地夹具约 40 ms，**但曾经是两次**，见 §8.3 |
| `ANALYSIS_TOKEN` 的对照样本（§7 第 4 条） | 「MCP 不放大 token，反而可能减少」 | **方向相反，而且三次都是**，见下表 |

`ANALYSIS_TOKEN` 三组对照（`cache｜uncached｜output`）：

| 来源 | 不挂 MCP | 挂上 MCP | uncached 之差 |
|---|---|---|---|
| 步骤六手工那次 | `7424｜111｜223` | `11648｜411｜304` | **+300** |
| 收尾第一轮回归 | `11264｜237｜299` | `11648｜399｜295` | **+162** |
| 收尾第二轮回归（全量） | `11520｜239｜455` | `11648｜418｜324` | **+179** |

**三次都是挂上更贵，且贵在 uncached input** —— 工具 schema 进了每一轮 prompt，这一项稳定多出
一百几十到三百。`output` 那一列三次方向不一（`+81`、`-4`、`-131`），是生成文本本身的波动，
读不出结论。§7 第 4 条那条待决事项已按这个结果重写。

### 8.6 收尾的两轮全量回归

**第一轮**，2026-08-16，`SKIP_HOSTILE=1 bash deploy/test/verify.sh`（六个服务起着、真 DeepSeek、
夹具由脚本自己起停）：**54 条里 53 条通过，1 条未验**，退出码 `2`。

- **P0–P9 那 47 条一条都没被打穿** —— 包括历来最容易偶发的 `P2①`（这一轮真砍到了）与
  要四次审批往返的 `P3①`。§6 那张回归关系表里点名的六处（`AgentConfig` 加字段、`_graph()` 多传
  `tools=`、子智能体工具栈、`agent_versions` 加列、`ResourceKind` 加 `MCP`、`RunTask` 形状）
  **各自对应的判据全绿**；其中 `ResourceKind` 那一处是唯一「会打穿且是静默的」，它由 `P7⑤`
  与 `P8⑤` 守着，两条都过。
- **P10 七条全过**，含两条要花钱的：`P10①` 双向断言 + 快照 + 三个观察项，`P10⑦` 真外网
  `mcp.deepwiki.com` 的 `read_wiki_structure` 返回 1843 字节、`status=success`。
- **未验的是 `P1①`**（四条破坏性测试要交互式 sudo，跑不进无 tty 的会话）——
  **记「未验」而不是「通过」**，与前几期同一条规矩。

**第二轮**，同日在有 tty 的终端里 `bash deploy/test/verify.sh`（不带任何开关，破坏性那组一并跑）：
**54 条全过，退出码 `0`** —— 本期收尾的最终结果。

- **`P1①` 四条破坏性补验通过**：死循环被 `--cpus=1` 限住（101.09%）、fork 炸弹被 `--pids-limit=128`
  挡住（宿主进程 745 → 751）、`/workspace` 在 5120MB 处 ENOSPC 且容器销毁重建后配额仍在、
  `/tmp` 在 512m 处 ENOSPC。宿主机内存（19327 → 19019 → 回基线）、磁盘（30025 → 30025）、
  进程数都回到基线。**这是 P10 改动之后头一次连破坏性那组一起验完**。
- 第一轮已过的 53 条**第二轮又各自过了一遍**，两轮之间没有偶发翻转 —— 含 `P2①`（第五个样本，
  工具调用 3 / 返回 3，差值仍是 0，已记进 [P2 §7.1](./P2-plan.md)）。
- `P0` 这一轮的完整分析：3714 条事件断线重连不重不漏、产物 `outputs/各行业年化波动率.png`
  48108 字节、token `62080｜2906｜6111`。

### 8.7 上游文档的三处更正

§2 里推翻决策文档的那三处已回填到 [P6 决策](./P6-decision.md)（F1 的理由、F5 的落点、F6 的后果），
每处都写明「定案不变 / 定案怎么变」以及「不改的话下一个人会错在哪」。[接入规范 §5.4](../01design/04extension-integration.md)
的第 5、6 项按实测改准（比对不一致是**记 warn** 不是跳过整个 server；30 秒那道超时是平台自己包的），
并补了熔断口径那一段 —— 那份文档是给校外服务作者看的，写错等于让他们照着做错事。
[风险登记 §10.2.1](../01design/09risk-register.md) 的第二、第四条各补一段落地记录。

**§5.4 的 13 项交付物缩成 4 项这件事**（P6 决策 §12.1 记在 P10 名下）**在 2026-08-13 随上游文档
修订就已经做掉了**，本期只需改措辞。
