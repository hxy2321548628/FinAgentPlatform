...
**验收现状：免费的五条全过，两条待跑。** ① 与 ⑦ 要真实调用 DeepSeek，尚未执行 —— 见 §8.5。
...
`test/api/artifact_test.py::test_the_run_reports_ids_that_the_artifact_endpoint_accepts` 约 **1/15** 概率报红（产物列表为空）。

**它不在 P3 的改动路径上** —— 那段代码 P0/P1 就定型了，本期一行没动。已经排除的猜测：文件 mtime 的时钟精度（在实际用的文件系统上探了 3000 次，`mtime` 一次都没落在取样时刻之前）。加插桩之后连跑 65 次未能复现，插桩本身改变了时序。

**登记而不是绕过**：它是「全绿但产物为零」那一类的近亲，值得单独花时间查，但不该占用 P3 的实施时间。
...
| ⑥ 主动取消停得下来 | ✅ 已过 |
| ⑦ P2 验收六条不回归 | ⏸ **待跑**（转调 p2.sh，其中两条要 LLM） |

**脚本自己先验过能报红**：第一版跑出来就抓到两个自身的 bug —— psql 的 `RETURNING` 把 `INSERT 0 1` 那行状态粘在了 run id 后面；验 cache_read 那一步被放行的提交会被 worker 真的跑掉，既花钱又让紧接着的「`waiting_approval` 不占并发」数到一个 running。两个都改完才转的绿。

> **观察项（§4 要求，不设门槛）**：每次验收跑完把这一次分析的未命中 token 数记下来。①那四次审批各会打印一行，攒成分布之后回 `quota/policy.py` 校准初值 —— 那几个数现在是从**一个**样本外推的。
