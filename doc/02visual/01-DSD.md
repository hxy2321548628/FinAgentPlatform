...
- Workspace 侧栏：深色 `--ws-sidebar-bg`，宽 220px，导航项 13px，激活态
  `--ws-sidebar-accent` 底 + `--action` 3px 左沿；**hover 必须有反馈**（现为
  `--ws-sidebar-hover`，激活态不叠加 hover）。
- Thread 侧栏：浅色 `--surface`，宽 240px；「＋ 新建分析」主按钮贴顶。
...
| 组件 | 规格 |
|---|---|
| 用户气泡 | 右对齐，`--brand` 底白字，圆角 `12px 12px 2px 12px`，max-width 620px |
| 助手消息 | 左对齐，32px 圆角方块头像（`--brand` 底、白字 F）+ 白色气泡，圆角 `2px 12px 12px 12px` |
| 思考块 | `<details open>`，`--action-light` 底、`--action-border` 2px 左沿，标题「{来源} · 分析思路」 |
...
| 反馈 | 规格 |
|---|---|
| 加载中 | 先用文字「正在加载…」；P1 起换骨架屏（列表 3 行、卡片 1 组） |
| 空状态 | 居中文字 + 行动指引（如「还没有分析对话」），不渲染残缺 UI |
| 错误 | 行内 `role="alert"`，`--danger` 色，含下一步动作（如「请重试」） |
| 成功通知 | **P1 起统一 Toast**：右上角、3s 自动消失、`aria-live="polite"`、三态配色；现在散落的内联小字不再新增 |
| 危险操作 | 一律自定义确认弹窗（2.8），**禁止 `window.confirm`**；文案明确「不可撤销」（后端无恢复端点，不做假撤销） |

### 2.5 输入区

- 输入框为无边框 `<textarea>`，min-height 52px、max-height 160px、14px 字号，
  placeholder「输入分析需求…（Enter 发送，Shift+Enter 换行）」；
- 发送按钮：40×40px（触控目标 ≥40px）、`--action` 底、`aria-label="发送"`；
  运行中替换为同尺寸停止按钮（`--danger` 底、`aria-label="停止分析"`）；
- 配置入口为输入框上方 12px 文字按钮「配置智能体 · …」；配置弹窗规格见 2.7。
