...
四条判据加进了 [`verify.sh`](../../deploy/test/verify.sh) 的 `p5` 组，**一次跑过，32 条断言全绿**，
判据总数 22 → 26。同一轮里旧的 16 条免费判据也全过 —— **静态核对那句话被真跑证实了，
P5 改的三处没有打穿任何历史判据。**

| 判据 | 验的是什么 | 断言 |
|---|---|---|
| `P5①` | 自助注册与入组审批走得通，且邀请码不跟着公开列表外泄 | 11 |
| `P5②` | 工作目录五个端点走得通，符号链接与越界路径都进不来 | 9 |
| `P5③` | 会话增删改查与聊天历史走得通，提问原文一字不差地读得回来 | 7 |
| `P5④` | 删会话之后工作目录真的没了，而 `runs` 那几行留着 | 5 |

**四条全免费，一次模型调用都不花，因此不挂 `SKIP_LLM`。** P5 改的全是接口层，与模型无关。
`P5③` 要一条真 run 才有历史可读，而它**提交前先把标题填上** —— 标题为空时 `POST /runs` 会挂
一个后台任务去调轻量模型起名，填过就不挂了。提交完立刻取消，第二条历史直接写库补上
（为了验翻页而烧一次分析不值当，与 P3④ 造 run 是同一条理由）。

**每条都按「这次真的触发了要测的场景吗」写**，不是「这次没报错吗」：

- `P5①` 的可见性验的是**同一个端点、同一个组，组主拿得到邀请码而组员拿到的是空**；审批验的是
  **名册在批准前后 0 人 → 1 人**，不是端点回了 204。
- `P5②` 的符号链接那条**先确认链接真的建成了**再断言它不在树里 —— 不确认的话，一个根本没建出
  链接的环境照样绿，那是假绿。
- `P5④` 的判据是**磁盘上真的没了**，不是「列表里消失」。销毁失败时端点照样答 204（那是有意的
  取舍），光看状态码永远看不出孤儿目录。
- `P5③` 里 `agent_config` **只验存取，不验它对 agent 有没有用** —— 它从 `0003` 起就写得进读得出
  而装配层从没读过它一次，「配与不配，输出可见地不同」是 [P6 决策 §11](./P6-decision.md) 的判据，
  不是这一条的。脚本里就地写了这句注释，免得把这条绿当成那件事已经成立。
