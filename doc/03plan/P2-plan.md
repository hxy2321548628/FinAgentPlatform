...
| v0.3 | 2026-08-07 | hxy | **六个步骤全部实施完毕**，回填 §8：与计划的三处偏离、一项未做、三个部署才暴露的问题、验收现状（②③⑥ 已过，①④⑤ 待跑） |
...
| 项 | 状态 |
|---|---|
| ~~**工具幂等键要不要提前到本期**~~ | **已关闭：先量后定。** 步骤四把「崩溃恢复后工具重复执行了几次」量出来（验收标准⑤，只测量不设门槛），据实测决定当期做还是留 P3。这是 P0 用探针定 `checkpoint_ns` 的老办法 —— 不实测就定，是在猜。<br>背景：[ADR-0014](../01design/adr/0014-tool-idempotency-key.md) 的去重排在 P3，但本期引入的重投机制正是它要防的场景（[ADR-0008](../01design/adr/0008-langgraph-checkpointer.md) 明确崩在工具执行途中会让整节点重跑），落点已于 P1 就位。<br>**若实测确实重复执行，当期就把它落到 broker**<br>→ **量出来了，不提前，维持 ADR-0014 的 P3 落点**，见 §7.1 |
...
### 7.1 幂等键：实测结果与据此的决定（2026-08-07）

验收①跑了一次完整的崩溃恢复，`p2.sh` 打印的观察项是：

```
【观察项】工具调用 4 次、返回 4 次 —— 差值即崩在工具执行途中的重跑次数
```

**差值 0：本轮没有观测到任何工具重复执行。** 同一次里 `run.started` 只有 1 条，说明恢复走的确实是续跑而非重跑。

**据此维持原案：幂等键不提前，仍按 [ADR-0014](../01design/adr/0014-tool-idempotency-key.md) 排在 P3。**

**但这个 0 只是「没量到」，不是「不会发生」**，两条局限要写明白：

- **单次观测，且崩溃点是随机的。** `kill -9` 挑在 run 进入 `running` 之后约 20 秒落下，这一刀正好砍在两次工具之间。砍进 `execute` 执行途中的那一刀会是什么样，本轮没有覆盖到。
- **这个指标只看得见「有调用无返回」。** 若节点重跑时把整对 `tool_call`/`tool_result` 都重发一遍，差值仍是 0 —— 那种重复要靠比对工具入参才认得出来，本轮没有做。

**因此 P3 动手时不要拿这条当「不需要幂等键」的证据**，它只支持「本期不必为它停下来」。真要定论，需要在 `execute` 执行途中定点注入崩溃，那是一条要专门造的用例。

---

## 8. 实施回填（2026-08-07）

六个步骤已全部落地，每步一个提交。**验收六条全量跑过一轮**：五条一次通过，⑤ 首轮报红并修复，见 §8.4。
...
步骤四「四个必须想清楚的点」里的第四点 —— **worker 崩溃时按名回收 broker 侧的沙箱租约** —— **本期没有实现**。

现状仍是 P1 那个兜底：[`sandbox/pool.py`](../../app/sandbox/pool.py) 的 `_expire_lease` 按 30 分钟超时强制归还。后果是 worker 被 `kill -9` 之后，它占着的那个沙箱名额要等半小时才回到池子里。

**这不是遗漏而是取舍**：本期验收六条没有一条依赖它，而它要改 broker 的申请接口（带上持有者身份）与新增一个「按持有者回收」的端点，属于另一块独立的改动。**这笔债仍挂在 [P1 §1.3](./P1-plan.md) 名下，未偿还。**

### 8.3 五个单测抓不到的问题

前三个在 `docker compose up` 时暴露，第四个要跑到全量验收才现形，第五个连单测自己都是受害者：
...
第三条值得单记一笔：`p2.sh` 交付前把三处断言各改坏一次，确认三条都变红、退出码 1，再改回。没有这一步，②那一条会一直显示绿色。

第四条的实测数据是 worker 77 行带 `run_id`、api 0 行。修的时候一并把验收⑥ 的检查面从「只看 api」改成「api 与 worker 各自过滤」—— 原来的写法在拆分之后既会在可观测性完好时绿，也会在它坏掉时绿。同样先对未修的栈跑了一遍确认它真报红（api 红、worker 绿），部署后才转的绿。

**第五条是本期代价最大的一个**，它有三层后果，一层比一层难想到：

1. 每条用例开头的 `flushdb` 冲的是业务库 —— 开发机上正在跑的 run 的事件、排队中的任务一起没；
2. 部署栈里的 worker 因此收到 `NOGROUP` 直接退出（靠 `restart: unless-stopped` 重起后 `ensure_group` 自愈，但每轮门禁都会把它打崩一次）；
3. **测试投出去的任务被真的 worker 领走，那是要花钱的模型调用。** 实测泄漏了 24 次 run，合计 `cache_read` 208,405、未命中 401,880、输出 13,063 —— 未命中那一项约是一次 P0 验收的十倍。

发现它靠的是一条断言的失败详情：`test_a_failed_run_reports_failed` 拿到了 `succeeded`，把事件流打出来一看，里面是没人写过的真实中文推理。**换句话说，没有那次「把原始数据打出来」的动作，这个 bug 会一直以「偶发红」的面目留在仓库里。**

修法是把库号改写进 URL 而不是靠关键字覆盖，并在夹具里加一道闸：`flushdb` 之前先确认连的确实是 15 号库。这道闸拿掉修复验过一次，确实拦得住。

**一处仍未处理**：`runs` / `run_events` 两张表上测试与业务共用同一个库，没有 Redis 那样的分家。眼下不出问题（run id 是 uuid，互不覆盖），但测试数据会混进业务表。要不要给测试单开一个 database，留待决定。

### 8.4 验收现状（2026-08-07 全量跑过一轮）

```bash
bash deploy/test/p2.sh                        # 六条全跑，有 LLM 费用
SKIP_LLM=1 SKIP_P1=1 bash deploy/test/p2.sh   # 免费的三条，约 1 分钟
```

| 条目 | 状态 |
|---|---|
| ① `kill -9` worker 后从 checkpoint 续跑 | ✅ 已过。它顺带量出来的那个数与据此的决定在 §7.1 |
| ② 三进程重启后 run 终态与事件流都还在 | ✅ 已过 |
| ③ 两个消费者并行不重复也不丢 | ✅ 已过 |
| ④ P0 验收四条经新架构全过 | ✅ 已过 |
| ⑤ P1 验收六条不回归 | 首轮**未过** —— P1⑥ 报红，即 §8.3 第四行那个回归。已修，P1 ③④⑤⑥ 在修复后的构建上重跑全绿 |
| ⑥ 事件 id 契约不变 | ✅ 已过 |

**一处口径要写清楚**：P1 的 ①（四条破坏性测试，要 sudo）与 ②（P0 四条经 Nginx，要 LLM）是在**修复前**的构建上验的绿，修复后没有重跑。两者之间的差异只有两条日志语句（`RunSubmitter.submit` 与 SSE 端点各加一行），不触及沙箱与执行路径。**要拿这份验收当交付依据的话，这两条应当在当前构建上再跑一次。**
| ⑥ 事件 id 契约不变 | ✅ 已过 |

> **§7 的「幂等键先量后定」仍未关闭**：它的输入是验收①的观察项（崩溃恢复时工具重复执行了几次），而①还没跑。`p2.sh` 会把这个数打印出来并提示记回本文档。
