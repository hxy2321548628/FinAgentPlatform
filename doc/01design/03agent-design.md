...
| 项 | 为什么能推迟 | 2026-08-13 状态 |
|---|---|---|
| 提示词工程（措辞打磨、few-shot、金融领域知识注入） | 改一个字符串重启 worker 即可，不越过接口 | 仍未排期 |
| 子 agent 划分 | 见 §2.1。开启会让事件流出现嵌套（`ns` 变深），但契约（§5.2 映射表）已预留，届时是加实现不是改契约 | **已定案，排 P9** |
| skill 库 / 用户自定义提示词 / MCP | 平台侧 §1.2 本就不做本期 | **已定案**：提示词 P6、skill P8、MCP P10 |
| 效果评测体系 | 见 §7 | 仍未排期（P6 需求方明确排除） |
| 摘要式上下文压缩 | 见 §5.2 | 仍未排期 |

> **2026-08-13：五项里三项已由 [P6 决策文档](../03plan/P6-decision.md) 定案并排期。** 那份文档用 68 条回答了「怎么做」，本节只标状态，具体口径以它为准。
...
> **2026-08-13 补记：子 agent 排入 P9，形态已定案。** 上面那条理由**没有被推翻，而是被绕开了** —— 开子 agent 的动因不再是「把线性任务拆开」（那确实不划算），而是**让教师能调用别人共享的智能体**（[P6 决策 G1](../03plan/P6-decision.md)）。
>
> | 项 | 定案 |
> |---|---|
> | 类型 | **只做提示词类** —— 就是别人共享的那个 agent 实体，取它的 `{name, description, prompt}` 做成 `SubAgent` |
> | 层数 | **一层**。判据是 **`config.subagents` 非空的不可被选作子智能体** —— 一条规则同时买到一层限制、成环不可能、以及「场景不能当子智能体」 |
> | 数量 | 一次 run 最多 5 个（**猜的数字，列为观察项**：量一次真实分析里被调了几次） |
> | 沙箱 | **共享主 agent 的**（[ADR-0003](./adr/0003-sandbox-per-thread-lifecycle.md) 的既定结构，单独开会打破它） |
> | 模型 | **主模型**，不是辅模型。见 §2.3 |
> | 提示词 | 作者的原样用，平台在末尾自动接上环境契约那一段（[E5](../03plan/P6-decision.md)） |
>
> **四处必须实测、不能推断的未知全在这一期**：嵌套事件的真实 `ns` 形状（§5.2 的 `_map_path` 至今没有真实子图样本）、子图的 token 会不会带上来、嵌套中断能否被 `aget_state` 查到、子图的 `recursion_limit` 是各自计数还是共享父图。**P9 单独成期正是因为这四处。**
...
> **2026-08-13 更新：辅模型仍然无处可用，而且这次是被明确选掉的。**
>
> 上面这段写于「子 agent 本期不做」的前提下，暗含「等子 agent 开了辅模型就有用了」。**P9 开子 agent 时并没有用它** —— [P6 决策 G5](../03plan/P6-decision.md) 定了**子智能体用主模型**，理由是它干的也是分析活，降模型会降质量。
>
> 于是辅模型的两个预期用武之地：子 agent（**已排除**）、上下文摘要压缩（**仍未排期**）。**它现在唯一的真实用户是「起会话标题」那一次轻量调用**（`app/thread/title.py`）—— 概括一句话不需要主模型那份多步推理能力，这一处是真的在省钱，但它一次分析只发生一次。
>
> **[ADR-0009](./adr/0009-default-model-selection.md) 的「辅助模型下沉」因此仍然基本是占位。** 这不是错误，是这个决策的价值到现在都还没兑现 —— 写在这里免得下次又以为「等 X 做了它就生效了」。
>
> **随之而来的成本账要记住**：一次分析挂 3 个子智能体 ≈ 3 倍的主模型调用。**这让 P9 的 token 归因实测从「应该做」变成「必须做」** —— 计量若漏掉子图，教师的配额就形同虚设，而且是静默的。
...
> - **§3.2 的工具名冻结因此更重要了** —— 外部工具撞了这 8 个名字要在上架时拒掉（[F6](../03plan/P6-decision.md)）
> - **事件映射一行不用改**：`event/mapper.py` 按 `AIMessage.tool_calls` / `ToolMessage` 提取，与工具是谁提供的无关。要改的是前端，加一个「未知工具」通用渲染分支（[F7](../03plan/P6-decision.md)）
> - **「单次幂等」与「broker 去重」两列对 MCP 工具不适用** —— [ADR-0014](./adr/0014-tool-idempotency-key.md) 的幂等键只作用于 broker 侧的沙箱工具，MCP 侧本期不做（[F9](../03plan/P6-decision.md)），代价是**声明有写操作的 MCP 一律先不批**
