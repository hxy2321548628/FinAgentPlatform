...
**实现路径已实测可行**（§2 第 5 条）：在 `wrap_model_call` 里 `request.override(messages=[*request.messages, 尾部块])`。**关键是它不写 state** —— 块不进 checkpoint，历史里不会堆起十条互相矛盾的旧状态栏。

**缓存上安全，这一点可以结构性地论证**：本轮注入的块下一轮就被换掉，因此它**从不参与下一轮的公共前缀** —— 前缀仍是 `[系统提示词, 消息 1..k]`，与没有状态栏时一模一样。代价只有块自身的 token，每次模型调用按未命中价付一次。

> **预算必须当期钉死。** 按一次分析 17 次模型调用估，一个 200 token 的块合计约 3400 个未命中 token。**这笔账记在 P13 上**：它会拉低命中率这个比值，而命中率正是 P13 的主判据。不先定预算，P14 往块里加一节就会把 P13 验收时量到的数改掉。
>
...
| 成本 | `cache_hit_rate` | NUMERIC | `runs` 表派生 —— **A2 的主指标** | 现成 |
...
**整批（4 项 run-level score）**：`pass_rate`、`mean_cache_hit_rate`、`probe_coverage`、`judge_noise`（同题三副本的极差）。

**诊断信息（写进 trace metadata，不打分、不判优劣）**：`llm_rounds`、`tool_calls`、`tool_error_rate`、`recovery_rate`、`gave_up`、`redundant_call_rate`、`answer_chars`、`answer_cites_artifact`。用途是失败之后回答「卡在哪」。需要时随时能升格成 score，反过来很难。
...
3. **不做需要标注「最优解」的指标。** 工具选择正确率、参数正确率、实际步数／最优步数比这三项都要求每题标一条标准工具序列，而本场景解法本就不唯一（pandas 和 numpy 都对），标出来的「最优」是造题人的偏好不是事实。反面由 `tool_error_rate` 与 `redundant_call_rate` 覆盖，这两个不需要 ground truth。
...
- **粗箭头是三处会互相咬的**：A1→A2 与 A8→A2（压缩触发与尾部块的 token 预算都直接改缓存命中率，任一端改了另一端的数就得重测）、A4→A3（记忆与提示词同期上，四样东西一起动质量分，靠逐次提交逐次量来拆）。
...
**决策阶段结束，23 条已定。** 下一步是把它们展开成三份实施计划。

### 10.1 三期的入口条件

| 期 | 开工前还欠什么 |
|---|---|
| **P13** | **§10.2 剩下的两处实测**（第 3 条已完成）。另外 **A7 的错误码要排成本期第一步** —— 撞递归上限现在不可分辨，状态栏的判据写不出来。**还欠一条采集通路**：`compaction_count` 现在取不到（C3），A1 的主指标量不出来 |
| **P14** | 无。B1、B2 定案完整，复用的是已验收的 HITL 链路 |
| **P15** | **P13 量出的噪声带**（D5：方差压不住则整份验收标准改写）+ **A4 的记忆隔离口径**（本文档只指出必须回答，没有回答） |
