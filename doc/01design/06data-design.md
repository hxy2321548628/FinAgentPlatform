...
> **2026-08-14：P7 把上面这一段实现了，形状与预告一致，只有一处比预告更绕。**
>
> 关联表落地为 `resource_groups`（`resource_kind` + `resource_id` + `group_id`，复合主键），取值本期只有 `agent` 一个。
>
> **三档在库里不是一个三值枚举**：前两档是 `agents.visibility ∈ {private, group}`，第三档「平台目录」由「有一个版本在 `reviews` 里 `approved`」决定，不是 `agents` 上的一列。把它塞进同一个枚举会开出一条不该存在的状态转移 —— 作者把自己的东西改成「平台目录」。理由与两张状态图见 [P7 计划 §2](../03plan/P7-plan.md)。
>
> **「我能看见哪些」这条查询确实变重了，但没有变成 JOIN。** 共享那一半走的是相关子查询（`EXISTS`）：JOIN 到 `resource_groups` 上，一个共享给三个组的 agent 会在列表里出现三次，去重要靠有人记得加 `distinct`，而忘了加的症状是列表里有重复项、不报错。
>
> **过滤只写在 `preset/repository.py` 一处**，三个方法（`list_catalog` / `list_available` / `list_owned`），端点一个 `where` 都不拼；**提交侧解析引用走的是 `list_available` 同一条语句**，因此列表看得见什么就引用得到什么，一行都不多。
...
### 6.3.1 会话内容出平台：2026-08-13 主动放开

> **上面那条「管理员看得到用量、看不到会话内容」在平台自己的接口内仍然成立，但它已经不再是一条有效的保密边界** —— 因为同一份内容以另一条路整份离开了平台。

`GET /api/admin/usage` 与它背后的账本（`report/usage.py`）已撤除，用量改到 **Langfuse** 上看。Langfuse 记的是完整 trace：**教师的提问、上传数据被读进上下文的那部分内容、模型的每一轮答复，逐字都在里面。**

| | 撤除前 | 现在 |
|---|---|---|
| 「谁花了多少」 | `/api/admin/usage`，只出数字 | Langfuse 的 UI，按 `user_id` 聚合 |
| 谁能看到会话内容 | 没有人（连管理员都不行） | **任何能登录 Langfuse 的人** |
| 边界靠什么守 | 端点只 select 数字列 + 一条专门的用例 | **只剩 Langfuse 自己的账号体系** |

**这是一次主动接受的放开，不是疏漏。** 记在这里是为了它出事时查得到「当时知道」。同时登记在[风险登记 §10.2.3](./09risk-register.md)。

**三条仍然成立的事实**，别把它们一起丢掉：

1. **配额闸门没有改**。`quota/usage.py` 读的是 `runs` 表，与 Langfuse 无关 —— 教师被拦下时的判据仍在平台内，不依赖外部服务活着。
2. **`runs.tokens_*` 仍然逐条落库**。数据没有消失，消失的是读它的那个端点；将来要把用量页做回来，不需要迁移数据。
3. **Langfuse 挂掉不影响分析**。回调失败不会中断 run —— 但**也不会有任何东西告诉你它挂了**，因为告警已于同日撤除。发现方式只有「打开 Langfuse 发现最近没有新 trace」。
