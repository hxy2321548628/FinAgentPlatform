...
| 工具幂等键去重 | 至少一次投递会带来重复执行的窗口 | **待定，见 §7** —— 它与本期验收标准直接相关，不是纯粹的 P3 功能 |
...
按[项目约定](../../CLAUDE.md)先确认再写。三处的结论都需要回填到对应的上游文档。
...
**要回填**：[P1 §1.3](./P1-plan.md) 的 MinIO 一行改期次；[架构 §6.5](../01design/01architecture.md) 把「跟 MinIO 在 P2 落地」改成 P3。
...
**要回填**：[架构 §11](../01design/01architecture.md) 的 P2 一行补上元数据表；[§6.2](../01design/01architecture.md) 给 `sandboxes` 标注现状。
...
**要回填**：[架构 §6.5](../01design/01architecture.md) 把「P2 一并关闭」改成上表的分工。
...
> **本期新增一条运维前提**：Postgres 的数据卷必须落在**宿主机的持久化卷**上，不能用匿名卷。否则 `docker compose down -v` 一次就把 checkpoint 全清了 —— 而那正是本期要保住的东西。这条要写进 [架构 §8.5](../01design/01architecture.md)。
...
| 项 | 内容 |
|---|---|
| 产出 | `runs` 表 + 建表方式 + 一层仓储；`RunExecutor` 内存里的 dict 退役 |
...
**建表方式要在这一步定下来**：是引入迁移工具（Alembic）还是启动时执行幂等 DDL。本期只有两张表，但 P3 要加五张 —— 定得草率，P3 会付账。这一项列入 §7。
...
| 项 | 状态 |
|---|---|
| **工具幂等键要不要提前到本期** | [ADR-0014](../01design/adr/0014-tool-idempotency-key.md) 的去重排在 P3，但**本期引入的重投机制正是它要防的那个场景**：验收标准①要 `kill -9` worker，而 [ADR-0008](../01design/adr/0008-langgraph-checkpointer.md) 明确崩在工具执行途中会让整节点重跑。<br>**建议：先量后定。** 步骤四把「恢复后工具重复执行了几次」量出来（验收标准⑤），据实测决定当期做还是留 P3 —— 这也是 P0 用探针定 `checkpoint_ns` 的老办法。<br>**需确认**：若实测确实重复执行，是否接受在 P2 就把 [ADR-0014](../01design/adr/0014-tool-idempotency-key.md) 落到 broker（落点已于 P1 就位） |
| **建表方式：迁移工具还是幂等 DDL** | 本期只有两张表，启动时跑一段幂等 DDL 就够；但 P3 要加五张表且要改 `runs` 的 `user_id`，届时没有迁移工具会很难受。<br>**建议：本期就引入 Alembic。** 理由：两张表时引入的成本最低，而「等需要了再引入」意味着要给已有数据补写第一版迁移 —— 那比现在做贵。<br>**需确认**：是否接受为两张表引入一个迁移工具的复杂度 |
| **worker 副本数与 api 副本数** | [架构 §4.4](../01design/01architecture.md) 的骨架写的是 `api × 2`、`worker × 2~4`。验收标准③要验「两个副本不重复消费」，因此 worker 至少 2 个。<br>**建议：本期固定 worker × 2、api × 1。** 理由：api 多副本要解决的是 SSE 连接与网关可用性，那是 P3 上认证与限流之后才谈得上的话题；本期把副本数用在真正要验的地方（消费语义）。<br>**需确认**：api 单副本是否可接受 —— 代价是 api 重启期间 SSE 全断，但前端有 `Last-Event-ID` 自动补齐 |

> 三项都不影响步骤零。**幂等键**在步骤四之前定，**建表方式**在步骤二之前定，**副本数**在步骤四之前定。
