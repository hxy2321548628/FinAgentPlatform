...
### 1.5 账号（P3 之后，脚本自己办，不用你动手）

P3 起 `/api/*` 一律要会话 cookie。脚本每次跑都新造一个一次性教师号再登录，这件事在 [`deploy/test/session.sh`](../../../deploy/test/session.sh) 里，四个验收脚本共用。

**你要保证的只有一件**：跑脚本的这台机器够得着同一套 compose 栈的 docker。平台没有公开的注册端点（建号是管理员的事），造号得进 `api` 容器算口令哈希、进 `postgres` 插行。`acceptance.sh` 从 P3 起因此也要 docker，不再是「给个 BASE_URL 就能跑」。
...
**跑脚本的这个 shell 不必再 export §1.3 那两个变量** —— 脚本从跑着的 broker 容器身上把它们读回来。这不只是图省事：脚本在 ⑤ 会重建 broker，自己另猜一份默认值的话，重建出来的挂载路径可能与在跑的那套不一致，**而那种错不报错，只会让验收在错的目标上跑**。broker 没在跑时脚本会直接告诉你该敲哪几行。
...
拉起来用的是 `compose start` 而不是 `up`：后者会按 `compose.yml` 重新算一遍配置，把 §3.3 临时压下去的名额又还原成 20，⑤ 就再也占不满池子了。
...
**这条为什么免费**：④ 末尾那次申请已经把沙箱占住，⑤ 直接提交一个 run 让它排队。执行器是**先申请沙箱、后调模型**的，所以排队超时的 run 一个 token 都不会花。

为此脚本会把 broker 的 `SANDBOX_MAX_CONTAINER` 改成 1、`SANDBOX_QUEUE_TIMEOUT` 改成 90 秒，**跑完自动还原**（异常退出也会，还原写在 trap 里）。这件事**在 ③ 之前一次做完，中途不再动 broker** —— 重建 broker 会让旧进程优雅退出，而优雅退出会走 `pool.aclose()` 把沙箱全销毁；夹在 ④ 与 ⑤ 之间做，等于把 ④ 刚认领好的容器毁掉，⑤ 再从一个说不清的状态起步。跑完可以确认一下：
